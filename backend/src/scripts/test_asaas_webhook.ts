...
async function bootstrap() {
  try {
    ...
    const tenantResult = await dataSource.query(
      'SELECT id, name, status, "asaas_customer_id" FROM tenants LIMIT 1',
    );
    if (tenantResult.length === 0) {
      console.error(
        'No tenants found in database! Please run seed or create a tenant.',
      );
      await app.close();
      process.exit(1);
    }
    ...
    try {
      ...
      const verifiedResult = await dataSource.query(
      ...
      if (newStatus === 'active') {
        console.log(
          '✅ SUCCESS: Tenant status was successfully transitioned to "active" by the Webhook!',
        );
      } else {
