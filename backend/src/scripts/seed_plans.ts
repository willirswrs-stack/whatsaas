...
const plans = [
  // Starter
  { name: 'Starter', maxInstances: 3, price: 97.9, billingCycle: 'MONTHLY' },
  {
    name: 'Starter',
    maxInstances: 3,
    price: 264.33,
  ...
  // Pro
  { name: 'Pro', maxInstances: 5, price: 147.9, billingCycle: 'MONTHLY' },
  ...
  // Agency
  { name: 'Agency', maxInstances: 10, price: 247.9, billingCycle: 'MONTHLY' },
  {
    name: 'Agency',
    maxInstances: 10,
    price: 669.33,
...
async function seed() {
  ...
  for (const plan of plans) {
    ...
    if (!exists) {
      ...
      console.log(
        `✅ Plano CRIADO: ${plan.name} (${plan.billingCycle}) -> R$ ${plan.price}`,
      );
    } else {
      ...
      console.log(
        `🔄 Plano ATUALIZADO: ${plan.name} (${plan.billingCycle}) -> R$ ${plan.price}`,
      );
    }
  }
