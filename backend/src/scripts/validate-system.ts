...
async function run() {
  ...
  try {
    const recon = await request('/reconnection/status', {
      headers: authHeaders,
    });
    if (recon.status === 200) console.log('✅ Reconnection Endpoint: OK');
    else
      console.warn(
        `⚠️ Reconnection Endpoint Erro (${recon.status}) - Provável necessidade de restart.`,
      );
  } catch (e: any) {
    console.warn('⚠️ Reconnection Exception:', e.message);
  }
  ...
  try {
    ...
    console.error('❌ Bull Board Offline:', e.message);
  }
  ...
  try {
    ...
    console.error('❌ Instances Exception:', e.message);
  }
}
