...
async function bootstrap() {
  try {
    console.log(
      'Initializing Application Context for Instance Proxy Binding Test...',
    );
    ...
    console.log(
      'This should automatically provision a Webshare SOCKS5 proxy and link it.',
    );
    ...
    if (proxyResult.length === 0) {
      throw new Error(
        `FAILED: Allocated Proxy ID ${instance.proxyId} does not exist in DB!`,
      );
    }
    ...
    console.log(
      `   assignedInstanceId: ${proxy.assignedInstanceId || 'NULL (FAIL)'}`,
    );

    if (proxy.assignedInstanceId === instance.id) {
      console.log(
        '✅ SUCCESS: Bidirectional binding verified (instances.proxy_id <-> proxies.assignedInstanceId)',
      );
    } else {
      console.error(
        `❌ FAILURE: Proxy assignedInstanceId (${proxy.assignedInstanceId}) does not match Instance ID (${instance.id})`,
      );
    }
    ...
    console.log(
      'This should automatically release/free the SOCKS5 proxy in the DB.',
    );
    ...
    console.log(
      `   assignedInstanceId after deletion: ${releasedProxy.assignedInstanceId}`,
    );

    if (releasedProxy.assignedInstanceId === null) {
      console.log(
        '✅ SUCCESS: Proxy was successfully freed and returned to the pool!',
      );
    } else {
      console.error(
        '❌ FAILURE: Proxy is still bound to instance:',
        releasedProxy.assignedInstanceId,
      );
    }
