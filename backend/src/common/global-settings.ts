...
export const getGlobalApiSettings = (): GlobalApiSettings => {
  ...
  return {
    ...
    elevenlabsKey:
      storedSettings.elevenlabsKey || process.env.ELEVENLABS_API_KEY || '',
    evolutionUrl:
      storedSettings.evolutionUrl || process.env.EVOLUTION_API_URL || '',
    evolutionKey:
      storedSettings.evolutionKey || process.env.EVOLUTION_API_KEY || '',
    webhookUrl: storedSettings.webhookUrl || process.env.WEBHOOK_URL || '',
  };
};

export const updateGlobalApiSettings = (
  newSettings: Partial<GlobalApiSettings>,
) => {
  const current = getGlobalApiSettings();
