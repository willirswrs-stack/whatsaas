...
export class MetaGraphApiService {
  ...
  async getPhoneNumberInfo(
    ...
    try {
      ...
      const response = await client.get(`/${phoneNumberId}`, {
        params: {
          fields:
            'display_phone_number,verified_name,quality_rating,platform_type,throughput',
        },
      });
      return response.data;
    } catch (error) {
      ...
      return [
        {
          ...
          components: [
            ...
            {
              type: 'BODY',
              text: 'Seja bem-vindo à nossa plataforma. Seu código é {{1}}.',
            },
          ],
        },
        {
        ...
        },
        {
          ...
          language: 'pt_BR',
          status: 'APPROVED',
          components: [{ type: 'BODY', text: 'Seu código de acesso é {{1}}.' }],
        },
      ];
    }
    try {
      ...
      while (nextUrl) {
        ...
        nextUrl = response.data.paging?.next
          ? response.data.paging.next.replace(
    ...
    try {
      ...
      const response = await client.get(`/${wabaId}/message_templates`, {
        params: {
          name: templateName,
          fields:
            'id,name,category,language,status,components,quality_score,rejected_reason',
        },
      });
  ...
  async uploadMedia(
    ...
    try {
      ...
      if (
        ...
        mediaUrl.startsWith('http://host.docker.internal')
      ) {
  ...
  private handleApiError(error: any, operation: string): never {
    ...
    this.logger.error(
      `Meta API Error [${operation}]: ${errorMessage} (Code: ${errorCode})`,
    );
