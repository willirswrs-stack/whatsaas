...
export class MetaTemplatesController {
  ...
  async createAccount(
    ...
    console.log(
      '[DEBUG-CTRL] Received createAccount request:',
      JSON.stringify(dto, null, 2),
    );
    try {
      return await this.metaTemplatesService.createWabaAccount(
        req.user.tenantId,
        dto,
      );
    } catch (e) {
  ...
  async deleteAccount(@Request() req, @Param('id') id: string): Promise<void> {
  ...
  async deleteTemplate(
    ...
    await this.metaTemplatesService.deleteTemplate(
      req.user.tenantId,
      id,
      templateName,
    );
