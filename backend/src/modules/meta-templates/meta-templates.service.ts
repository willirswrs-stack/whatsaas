...
export class MetaTemplatesService {
  ...
  async createWabaAccount(
    tenantId: string,
    dto: CreateWabaAccountDto,
  ): Promise<WabaAccount> {
    this.logger.log(`Creating WABA account for tenant ${tenantId}`);

    // Validate access token by fetching phone info
    try {
      console.log('[DEBUG] Validating Meta Token...', {
        phoneNumberId: dto.phoneNumberId,
      });
      const phoneInfo = await this.metaGraphApiService.getPhoneNumberInfo(
        dto.phoneNumberId,
        dto.accessToken,
      );
      ...
      this.logger.error(
        `Failed to create WABA account: ${error.message}`,
        error.stack,
      );
      ...
      if (error.message && error.message.includes('does not exist')) {
        throw new BadRequestException(
          'ID do Telefone inválido. Verifique se você não inverteu "Phone Number ID" com o "Número de Telefone". O ID deve conter apenas números fornecidos pela Meta.',
        );
      }

      throw new BadRequestException(
        `Erro na API Oficial (Meta): ${error.message || 'Erro desconhecido'}`,
      );
    }
  }
  ...
  async getBusinessProfile(
    ...
    return this.metaGraphApiService.getBusinessProfile(
      account.phoneNumberId,
      accessToken,
    );
  }
  ...
  async createTemplate(
    ...
    return this.metaGraphApiService.createTemplate(
      account.wabaId,
      accessToken,
      {
        name: normalizedName,
  ...
  async deleteTemplate(
    ...
    return this.metaGraphApiService.deleteTemplate(
      account.wabaId,
      accessToken,
      templateName,
    );
