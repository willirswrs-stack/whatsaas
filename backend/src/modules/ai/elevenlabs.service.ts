...
export class ElevenLabsService {
  ...
  async cloneVoice(
    ...
    if (!apiKey || apiKey.length < 5) {
      throw new Error(
        'Chave da ElevenLabs não configurada (nem nas Integrações IA, nem no .env).',
      );
    }
    ...
    try {
      ...
      form.append(
        'description',
        'Cloned automatically via WhatSaas Warmup Engine',
      );
      ...
      this.logger.log(
        `[ElevenLabs] ✅ Voz clonada com sucesso! ID: ${voiceId}`,
      );
      return voiceId;
    } catch (error: any) {
      this.logger.error(
        `[ElevenLabs] Erro na clonagem: ${error.response?.data ? JSON.stringify(error.response.data) : error.message}`,
      );
      throw new Error(
        `Erro ElevenLabs: ${error.response?.data?.detail?.message || error.message}`,
      );
    }
  }
