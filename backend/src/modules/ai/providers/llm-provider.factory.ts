  ...
  async generateWithFallback(
    ...
    if (preferredProvider && preferredProvider.isConfigured()) {
      try {
        ...
        this.logger.warn(
          `Provider preferencial '${preferredType}' falhou (${error.message}). Iniciando fallbacks em cascata...`,
        );
      }
    } else {
      this.logger.warn(
        `Provider preferencial '${preferredType}' não está configurado. Iniciando fallbacks em cascata...`,
      );
    }
    ...
    for (const fallbackProvider of allProviders) {
      ...
      if (fallbackProvider.isConfigured()) {
        try {
          this.logger.log(
            `Tentando fallback com o provedor: ${fallbackProvider.id}...`,
          );
          ...
          this.logger.warn(
            `Fallback '${fallbackProvider.id}' falhou: ${fallbackError.message}`,
          );
        }
      }
    }
    ...
    throw new Error(
      'Todos os provedores de LLM falharam ou não estão configurados adequadamente.',
    );
  }
