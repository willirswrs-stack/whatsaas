...
export class GroqAdapter implements ILLMProvider {
  ...
  getAvailableModels(): LLMModelInfo[] {
    return [
      // Llama 3.3
      {
        ...
        name: 'Llama 3.3 70B',
        description: 'Mais recente e versátil',
  ...
  async generate(
    ...
    if (!this.apiKey) {
      throw new Error(
        'Groq não configurado. Configure a API key nas configurações.',
      );
    }
    ...
    const doRequest = async (modelToUse: string): Promise<LLMResponse> => {
      ...
      this.logger.log(
        `✅ Groq gerou resposta com ${modelToUse} (${tokensUsed} tokens)`,
      );
    ...
    try {
      ...
      if (error.statusCode === 429 && model !== 'llama-3.1-8b-instant') {
        this.logger.warn(
          `Groq rate limit atingido no modelo '${model}'. Tentando fallback com 'llama-3.1-8b-instant'...`,
        );
        try {
          return await doRequest('llama-3.1-8b-instant');
        } catch (fallbackError: any) {
          this.logger.error(
            `Groq fallback também falhou: ${fallbackError.message}`,
          );
          throw fallbackError;
        }
      }
