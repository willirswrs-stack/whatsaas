...
export class OpenAIAdapter implements ILLMProvider {
  ...
  getAvailableModels(): LLMModelInfo[] {
    return [
      ...
      },
      {
        id: 'o1-mini',
        name: 'o1 Mini',
        description: 'Raciocínio eficiente',
        contextWindow: 128000,
        maxOutput: 65536,
      },
    ];
  ...
  async generate(
    ...
    if (!this.client) {
      throw new Error(
        'OpenAI não configurado. Configure a API key nas configurações.',
      );
    }
    ...
    try {
      ...
      this.logger.log(
        `✅ OpenAI gerou resposta (${response.usage?.total_tokens || 0} tokens)`,
      );
