...
export class GroupWarmupService {
   ...
   */
  async processGroupWarmupForInstance(
    instance: Instance,
    ...
    try {
      ...
      if (!isEnabled) {
        return {
          success: false,
          action: 'skip',
          message: 'Group warmup not enabled',
        };
      }
      if (!niche || niche.trim().length < 3) {
        return {
          success: false,
          action: 'skip',
          message: 'No valid niche defined for group search',
        };
      }
      if (currentDay < minDay) {
        return {
          success: false,
          action: 'skip',
          message: `Maturity insufficient (${currentDay}/${minDay} days)`,
        };
      }
      ...
      if (hasGroups && Math.random() < 0.4) {
        this.logger.log(
          `[GroupWarmup] 📰 Choosing 'Share News' strategy today for instance ${instance.instanceName}`,
        );
        const shareRes = await this.searchAndShareNicheNews(instance);
        return {
          success: shareRes.success,
          action: 'share_news',
          message: shareRes.message || `Shared news: ${shareRes.url}`,
        };
      }

      // Check if we already performed a join activity in the last 20 hours to throttle
      const lastJoin = meta.lastGroupJoinAt
        ? new Date(meta.lastGroupJoinAt)
        : null;
      if (lastJoin && Date.now() - lastJoin.getTime() < 20 * 60 * 60 * 1000) {
        return {
          success: false,
          action: 'throttle',
          message: 'Group join throttled (1 per 24h)',
        };
      }

      this.logger.log(
        `[GroupWarmup] 🕵️ Triggering group hunt for instance ${instance.instanceName} in niche: "${niche}"`,
      );
      ...
      let potentialLinks: string[] = [
        ...customLinks,
        ...(meta.foundGroupLinksCache || []),
      ];

      const lastSearch = meta.lastGroupSearchAt
        ? new Date(meta.lastGroupSearchAt)
        : null;
      ...
      const needsScraping =
        ...
        (!customLinks.length &&
          (!lastSearch ||
            Date.now() - lastSearch.getTime() > 3 * 24 * 60 * 60 * 1000));
      ...
      if (needsScraping) {
        ...
        this.logger.log(
          `[GroupWarmup] Search complete. Found ${scraped.length} new potential groups via fallback search.`,
        );
      }
      ...
      if (candidateLinks.length === 0) {
        return {
          success: false,
          action: 'no_links',
          message: 'No new group links available to join',
        };
      }
      ...
      this.logger.log(
        `[GroupWarmup] 🚪 Attempting to join group: ${inviteUrl}`,
      );
      ...
      const joinResult = await provider.joinGroup(
      ...
      if (joinResult) {
        ...
        newGroupJid =
          joinResult.jid || joinResult.groupJid || joinResult.data?.jid || '';
      }
      ...
      try {
        ...
        this.logger.warn(
          `Failed to generate custom AI group intro: ${aiErr.message}`,
        );
      }
      ...
      this.logger.log(
        `[GroupWarmup] Waiting ${Math.round(typingMs / 1000)}s before sending intro message...`,
      );
      ...
      setTimeout(async () => {
        try {
          ...
          if (newGroupJid) {
            ...
            this.logger.log(
              `[GroupWarmup] Intro message successfully sent to group: ${newGroupJid}`,
            );
          } else {
            this.logger.log(
              `[GroupWarmup] Group joined, but JID not returned. Skipping immediate intro message.`,
            );
          }
        } catch (sendErr) {
          this.logger.warn(
            `Failed to send group intro message, but join was successful: ${sendErr.message}`,
          );
        }
      }, typingMs);
   ...
   */
  async handleGroupIncomingMessage(
    instanceName: string,
    groupJid: string,
    content: string,
  ): Promise<void> {
    try {
      ...
      const hasKeywords = nicheKeywords.some((kw) =>
        lowercaseContent.includes(kw),
      );
      ...
      this.logger.log(
        `[GroupWarmup] 🤖 Decision hit! Responding reactively to group ${groupJid} on instance ${instanceName}.`,
      );
...
Mensagem no grupo: "${content}"`;
      ...
      this.logger.log(
        `[GroupWarmup] Simulating reactive group typing for ${Math.round(delayMs / 1000)}s...`,
      );
      ...
      setTimeout(async () => {
        try {
          ...
          this.logger.log(
            `[GroupWarmup] Dispatched reactive comment to group ${groupJid}`,
          );
        } catch (err) {
      ...
      this.logger.error(
        `Failed inside handleGroupIncomingMessage: ${error.message}`,
      );
    }
  }
  ...
  async searchAndShareNicheNews(
    ...
    try {
      ...
      if (!targetJid) {
        return {
          success: false,
          message: 'No joined groups available for sharing news.',
        };
      }

      this.logger.log(
        `[GroupWarmup] 📰 Commencing Google News RSS lookup for niche: "${niche}"`,
      );
      ...
      if (newsItems.length === 0) {
        return {
          success: false,
          message: 'DuckDuck/Google News RSS returned zero relevant items.',
        };
      }
...
Não coloque o título nem o link na sua resposta, apenas essa introdução.`;
      ...
      this.logger.log(
        `[GroupWarmup] Successfully curated content to group ${targetJid}`,
      );

      return { success: true, url: pick.link };
    } catch (err) {
      this.logger.error(
        `Failed inside searchAndShareNicheNews: ${err.message}`,
      );
  ...
  async searchGroupLinks(niche: string): Promise<string[]> {
    try {
      ...
      this.logger.log(
        `[GroupWarmup] Engine found ${results.length} unique group links for "${niche}"`,
      );

      return results.sort(() => Math.random() - 0.5);
    } catch (e) {
      this.logger.warn(
        `WebSearch Engine failed to scrape groups: ${e.message}`,
      );
      return [];
    }
  }
