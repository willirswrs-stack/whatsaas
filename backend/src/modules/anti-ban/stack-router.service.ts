...
const ROUTING_RULES: RoutingRule[] = [
  ...
  },
  {
    ...
    priority: 41,
    condition: (input) =>
      input.chipHealthScore >= 50 && input.chipHealthScore < 85,
    stack: 'waha',
    confidence: 78,
...
export class StackRouterService {
  ...
  route(input: StackRoutingInput): StackRoutingResult {
    ...
    const sortedRules = [...ROUTING_RULES].sort(
    ...
    const fallbackStack = this.determineFallback(
      matchedRule.stack,
      enrichedInput,
    );
  ...
  recommendForScenario(scenario: {
    ...
    notes.push(
      `Baseado em: warmup=${warmupDays}d, health=${health}, volume=${volume}`,
    );
  ...
  private validateInput(input: StackRoutingInput, warnings: string[]): void {
    if (input.chipHealthScore < 0 || input.chipHealthScore > 100) {
      warnings.push(
        `chipHealthScore fora do range (${input.chipHealthScore}), ajustado`,
      );
    ...
    if (input.warmupDays < 0) {
      warnings.push(
        `warmupDays negativo (${input.warmupDays}), ajustado para 0`,
      );
      input.warmupDays = 0;
    }
  ...
  private adjustConfidence(
    ...
    if (
      input.averageDeliveryRate !== undefined &&
      input.averageDeliveryRate > 95
    ) {
      adjustment += 5;
    }
  ...
  private summarizeInput(input: StackRoutingInput): string {
    return (
      `warmup=${input.warmupDays}d, health=${input.chipHealthScore}%, ` +
