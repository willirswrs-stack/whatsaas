...
export class AntiBanAnalyticsService {
  ...
  getStackMetrics(stack?: StackType): StackMetrics | StackMetrics[] {
    if (stack) {
      return (
        this.metricsCache.stacks.get(stack) ||
        this.createEmptyStackMetrics(stack)
      );
    }
  ...
  getDashboardData(): DashboardData {
    ...
    const deliveredEvents = recentEvents.filter(
  ...
  getStackSuccessRates(
    ...
    const rates: Record<
      StackType,
      { sent: number; success: number; rate: number }
    > = {
  ...
  private updateInstanceMetrics(event: MessageEvent): void {
    ...
    if (event.durationMs) {
      const totalDuration =
        metrics.averageDeliveryTimeMs * (metrics.totalSent - 1) +
        event.durationMs;
  ...
  private updateCampaignMetrics(event: MessageEvent): void {
    ...
    if (metrics.totalContacts > 0) {
      metrics.progressPercent =
        ((metrics.sent + metrics.failed) / metrics.totalContacts) * 100;
    }
  ...
  private updateStackMetrics(event: MessageEvent): void {
    ...
    const stackEvents = this.eventStore.events.filter(
      (e) => e.stack === event.stack,
    );
    ...
    const delivered = stackEvents.filter(
      (e) => e.status === 'delivered',
    ).length;
    ...
    metrics.lastHourVolume = stackEvents.filter(
  ...
  private detectAnomalies(event: MessageEvent): void {
    ...
    if (
      previousDeliveryRate !== null &&
      previousDeliveryRate - metrics.deliveryRate >=
        this.thresholds.deliveryDropPercent
    ) {
      this.createAlert({
        type: 'delivery_drop',
        severity: 'warning',
  ...
  private calculateHealthTrends(): HealthTrend[] {
    ...
    for (const [instanceId, history] of this.healthHistory.entries()) {
      ...
      const trend = {
        ...
        trend:
          this.calculateTrend(instanceId) === 'improving'
            ? ('up' as const)
  ...
  private getPreviousDeliveryRate(instanceId: string): number | null {
    ...
    const delivered = previousMetrics.filter(
      (e) => e.status === 'delivered',
    ).length;
  ...
  exportPrometheusMetrics(): string {
    ...
    for (const instance of allInstances) {
      lines.push(
        `whatsaas_instance_sent_total{instance="${instance.instanceId}"} ${instance.totalSent}`,
      );
      lines.push(
        `whatsaas_instance_delivered_total{instance="${instance.instanceId}"} ${instance.totalDelivered}`,
      );
      lines.push(
        `whatsaas_instance_failed_total{instance="${instance.instanceId}"} ${instance.totalFailed}`,
      );
      lines.push(
        `whatsaas_instance_blocked_total{instance="${instance.instanceId}"} ${instance.totalBlocked}`,
      );
      lines.push(
        `whatsaas_instance_health_score{instance="${instance.instanceId}"} ${instance.healthScore}`,
      );
      lines.push(
        `whatsaas_instance_delivery_rate{instance="${instance.instanceId}"} ${instance.deliveryRate}`,
      );
    }
    ...
    for (const stack of this.metricsCache.stacks.values()) {
      lines.push(
        `whatsaas_stack_messages_total{stack="${stack.stack}"} ${stack.totalMessages}`,
      );
      lines.push(
        `whatsaas_stack_success_rate{stack="${stack.stack}"} ${stack.successRate}`,
      );
      lines.push(
        `whatsaas_stack_latency_ms{stack="${stack.stack}"} ${stack.averageLatencyMs}`,
      );
    }
