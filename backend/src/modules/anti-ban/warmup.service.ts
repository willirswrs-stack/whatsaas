...
export class WarmupService {
  ...
  async scheduleContinuousJob() {
    ...
    this.logger.log(
      '⏰ Continuous Warmup Routine scheduled for every 15 minutes (anti-ban protection)',
    );
  }
   ...
   */
  async executeDailyWarmupRoutine(): Promise<{
    advanced: number;
    completed: number;
    sessions: number;
  }> {
    ...
    for (const instance of instances) {
      ...
      if (instance.status === 'connected') {
        ...
        this.groupWarmupService
          .processGroupWarmupForInstance(instance)
          .then((res) => {
            if (res.success)
              this.logger.log(
                `[WarmupRoutine] Grupos para ${instance.instanceName}: ${res.message}`,
              );
          })
          .catch((e) =>
            this.logger.warn(
              `[WarmupRoutine] Falha no aquecimento de grupos de ${instance.instanceName}: ${e.message}`,
            ),
          );
      }
    }
    ...
    this.logger.log(
      `✅ Warmup Routine Finished: ${advancedCount} advanced, ${completedCount} completed.`,
    );
   ...
   */
  async executeContinuousWarmupRoutine(): Promise<{
    ...
    try {
      ...
      this.logger.log(
        `💬 ${sessionsTriggered} Global Warmup Sessions triggered dynamically`,
      );
    } catch (err) {
   ...
   */
  async advanceInstanceWarmup(
    instance: Instance,
  ): Promise<'advanced' | 'completed' | 'no-change'> {
    // Validation: If disconnected or banned, do not advance
    if (instance.status !== 'connected') {
      this.logger.warn(
        `Skipping warmup advance for ${instance.instanceName} (Status: ${instance.status})`,
      );
      return 'no-change';
    }
    ...
    if (!schedule) {
      ...
      this.logger.log(
        `🎓 Instance ${instance.instanceName} has graduated from warmup (but keeping it warm)!`,
      );
    }
  ...
  async createGlobalWarmupSessions() {
    ...
    const candidates = allInstances.filter(
      (i) =>
        (i.status === InstanceStatus.CONNECTED ||
          i.status === ('connected' as any)) &&
        i.phone &&
    ...
    if (candidates.length < 2) {
      this.logger.log(
        `Not enough candidates for global warmup right now (Found ${candidates.length} under limit)`,
      );
    ...
    const activeCandidates = candidates.filter((i) => {
      ...
      const isSleeping =
        currentMinutes >= sleepStart || currentMinutes < wakeUp;
      return !isSleeping;
    });

    if (activeCandidates.length < 2) {
      this.logger.log(
        `A maioria das instâncias está dormindo. Ativas: ${activeCandidates.length}`,
      );
      ...
      while (partnersSelected < maxPartners && attempts < maxPartners * 10) {
        ...
        if (partner) {
          ...
          const remainingLimit = Math.max(
            0,
            (chip.dailyLimit || 50) - (chip.dailySent || 0),
          );
    ...
    if (selectedPairs.length > MAX_PAIRS_PER_CYCLE) {
      this.logger.warn(
        `⚠️ [ANTI-BAN] Limitando pares de ${selectedPairs.length} → ${MAX_PAIRS_PER_CYCLE} por ciclo para segurança`,
      );
    }
    ...
    for (const [instA, instB] of limitedPairs) {
      try {
        ...
        const result = await this.createWarmupSession(
          instA.tenantId || 'system',
          instA,
          instB,
          baseDelay,
        );
        ...
        this.logger.error(
          `Session error ${instA.instanceName}↔${instB.instanceName}: ${err.message}`,
        );
      }
    }
  ...
  async createWarmupSession(
    ...
    if (!resolvedA || !resolvedB) {
      ...
      if (candidates.length < 2) {
        this.logger.warn(
          `Not enough candidates for warmup session (Found ${candidates.length}, need 2)`,
        );
        return {
          success: false,
          reason: 'min_instances',
    ...
    if (!instA.phone || !instB.phone) {
      this.logger.warn(
        `Instances missing phone numbers: A=${instA.phone}, B=${instB.phone}`,
      );
    ...
    const niche =
      instA.metaConfig?.warmupNiche || instB.metaConfig?.warmupNiche || null;
    ...
    try {
      ...
      if (!conversation || !Array.isArray(conversation)) {
        this.logger.error(
          `AI generated invalid conversation: ${JSON.stringify(conversation)}`,
        );
        return {
          success: false,
          reason: 'ai_error_invalid_response',
          totalDurationMs: 0,
        };
      }
    } catch (error) {
      this.logger.error(
        `Failed to generate warmup conversation: ${error.message}`,
      );
      return {
        success: false,
        reason: 'ai_exception',
    ...
    for (const msg of conversation) {
      try {
        if (!msg || typeof msg.content !== 'string') {
          this.logger.warn(
            `Skipping invalid message object: ${JSON.stringify(msg)}`,
          );
          continue;
        }
    ...
    console.log(
      `[Warmup] ✅ Scheduled ${messagesScheduled} messages for ${instA.instanceName}↔${instB.instanceName}. Duration: ~${minutes} min.`,
    );
    return {
      success: true,
      ...
      instA: {
        id: instA.id,
        name: instA.instanceName,
        phone: instA.phone,
        provider: instA.provider,
      },
      instB: {
        id: instB.id,
        name: instB.instanceName,
        phone: instB.phone,
        provider: instB.provider,
      },
    };
  }
  ...
  async createLiveSession(
    ...
    if (instA.status !== 'connected' || instB.status !== 'connected') {
      ...
      if (instA.status !== 'connected')
        disconnected.push(instA.instanceName || instA.phone);
      if (instB.status !== 'connected')
        disconnected.push(instB.instanceName || instB.phone);

      this.logger.warn(
        `[LIVE] Tentativa de iniciar conversa com instâncias desconectadas: ${disconnected.join(', ')}`,
      );
      return {
        success: false,
    ...
    this.logger.log(
      `🔴 [LIVE] Starting live session: ${instA.instanceName} ↔ ${instB.instanceName}`,
    );
  ...
  private async sendLiveMessagesAsync(
    ...
    for (const msg of conversation) {
      ...
      const currentA = await this.instanceRepo.findOne({
        where: { id: instA.id },
      });
      const currentB = await this.instanceRepo.findOne({
        where: { id: instB.id },
      });

      if (
        !currentA ||
        currentA.status !== 'connected' ||
        !currentB ||
        currentB.status !== 'connected'
      ) {
        this.logger.warn(
          `[LIVE] Aborting live session mid-way! One of the instances disconnected. A: ${currentA?.status}, B: ${currentB?.status}`,
        );
      ...
      const currentMsgContent = msg.content;
      ...
      try {
        ...
        try {
          ...
          if (senderStatus.status !== 'connected') {
            throw new Error(
              `Provider reports sender ${sender.instanceName} is not connected (${senderStatus.status})`,
            );
          }
        } catch (e) {
        ...
        await this.instanceRepo
          .increment({ id: sender.id }, 'dailySent', 1)
          .catch(() => {});
      } catch (e) {
    ...
    this.logger.log(
      `🔴 [LIVE] Session ${sessionId} complete — ${index} messages sent.`,
    );
  }
  ...
  getScheduleForInstance(instance: Instance, day: number) {
    ...
    const limit =
      day > maxDays
        ? MATURE_LIMIT
        : Math.floor(10 + clampedProgress * (3000 - 10));
    const interval = Math.max(5, Math.floor(120 - clampedProgress * (120 - 5)));
    const maxPartners = Math.floor(
      1 + Math.pow(clampedProgress, 1.5) * (30 - 1),
    );
  ...
  async getStats(tenantId: string) {
    ...
    for (const inst of instances) {
      if (inst.status === 'connected' && !inst.phone) {
        this.logger.log(
          `[SELF-HEALING] Instância conectada "${inst.instanceName}" está sem telefone no banco. Sincronizando em background...`,
        );
        this.instancesService.getStatus(inst.id, tenantId).catch((e) => {
          this.logger.warn(
            `Self-healing status sync falhou para ${inst.instanceName}: ${e.message}`,
          );
        });
      }
    }
    ...
    const totalMessagesSent = warmupInstances.reduce(
      (acc, curr) => acc + curr.dailySent,
      0,
    );
    ...
    const avgHealth =
      total > 0
        ? Math.round(
            warmupInstances.reduce(
              (acc, curr) => acc + (curr.status === 'connected' ? 100 : 50),
              0,
            ) / total,
          )
        : 0;
    ...
    return {
      ...
      instances: warmupInstances.map((i) => ({
        ...
        health:
          i.status === 'connected' ? 95 + Math.floor(Math.random() * 5) : 50,
      })),
    };
  }
