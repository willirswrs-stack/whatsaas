...
const DEFAULT_WEIGHTS: HealthScoreWeights = {
  ...
  blockHistory: 0.2, // 20% - Block history matters
  usageCapacity: 0.1, // 10% - Not overloaded
  activityRecency: 0.1, // 10% - Recently active
};
...
export class ChipHealthService {
  ...
  private healthCache: Map<string, { score: number; timestamp: Date }> =
    new Map();
   ...
   */
  async getTenantHealthOverview(tenantId: string): Promise<
    Array<{
      instanceId: string;
      instanceName: string;
      score: number;
  ...
  private calculateScoreFromInstance(instance: Instance): number {
    ...
    if (
      cached &&
      Date.now() - cached.timestamp.getTime() < this.cacheValidityMs
    ) {
      return cached.score;
    }
  ...
  private scoreToStatus(score: number): HealthStatus {
    if (score >= 85) {
      return {
        ...
        emoji: '🟢',
        recommendation:
          'Chip em excelente condição. Pode operar em capacidade máxima.',
    ...
    if (score >= 30) {
      return {
        ...
        emoji: '🔴',
        recommendation:
          'Chip com saúde baixa. Reduza significativamente o volume.',
        canSendMessages: true,
