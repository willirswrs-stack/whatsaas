...
export class InboxService {
  ...
  async saveMessage(dto: SaveMessageDto): Promise<Message | null> {
    try {
      ...
      const message = this.messageRepo.create({
        ...
        type: dto.type || 'text',
        status:
          dto.status || (dto.direction === 'inbound' ? 'received' : 'sent'),
        expiresAt,
      });
  ...
  async getConversations(
    ...
    const qb = this.messageRepo
      ...
      .groupBy(
        'm.remote_jid, m.remote_phone, m.remote_name, m.is_group, m.group_name, m.contact_id, m.instance_id, m.instance_name',
      )
    ...
    if (options?.instanceId) {
      qb.andWhere('m.instance_id = :instanceId', {
    ...
    if (options?.instanceId) {
      totalQb.andWhere('m.instance_id = :instanceId', {
        instanceId: options.instanceId,
      });
    }
  ...
  async sendReply(
    ...
    if (!instance) {
      throw new NotFoundException(
        'Nenhum chip conectado disponível para enviar a mensagem.',
      );
    }
