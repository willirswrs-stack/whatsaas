...
export class WebshareAdapter implements IProxyProvider {
  ...
  async buyOrAllocateProxy(
    ...
    this.logger.log(
      `[WEBSHARE ADAPTER] Iniciando alocação do pool Webshare para tenant ${tenantId}`,
    );

    if (!this.apiKey) {
      this.logger.warn(
        `[WEBSHARE ADAPTER] API Key da Webshare não configurada no arquivo .env!`,
      );
    ...
    try {
      ...
      this.logger.log(
        `[WEBSHARE ADAPTER] Encontrados ${webshareProxies.length} proxies no pool da Webshare.`,
      );
      ...
      const usedHostsPorts = new Set(
        currentProxies.map((p) => `${p.host}:${p.port}`),
      );

      // Selecionar o primeiro proxy retornado que ainda não existe no nosso banco de dados
      const availableProxy = webshareProxies.find(
        (p: any) => !usedHostsPorts.has(`${p.proxy_address}:${p.port}`),
      );

      if (availableProxy) {
        this.logger.log(
          `[WEBSHARE ADAPTER] Alocando novo proxy livre do pool: ${availableProxy.proxy_address}:${availableProxy.port}`,
        );
        return {
      ...
      this.logger.warn(
        `[WEBSHARE ADAPTER] Erro: Pool de proxies esgotado no Webshare (Todos os ${webshareProxies.length} IPs em uso).`,
      );
      throw new BadRequestException(
      ...
      this.logger.error(
        `[WEBSHARE ADAPTER] Erro ao consultar API da Webshare: ${error.message}`,
      );
      throw new BadRequestException(
