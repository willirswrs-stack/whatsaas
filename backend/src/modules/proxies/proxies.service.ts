...
export class ProxiesService {
  ...
  async assignProxy(
    ...
    this.logger.log(
      `Proxy ${proxyId} associado à instância ${instanceId || 'NENHUMA'}`,
    );
    return proxy;
  }
  ...
  async testProxy(data: {
    ...
    try {
      this.logger.log(
        `[PROXY TEST] Iniciando teste para: ${data.type}://${data.host}:${data.port}`,
      );
      let agent: any;
      ...
      this.logger.log(
        `[PROXY TEST] ✅ Sucesso! IP retornado: ${response.data?.ip} | Latência: ${latency}ms`,
      );
      ...
      return {
        ...
        latencyMs: latency,
        error:
          error.message || 'Tempo limite excedido ou proxy recusou a conexão.',
      };
    }
  }
  ...
  async buyProxyFromProvider(tenantId: string): Promise<ProxyEntity> {
    this.logger.log(
      `[PROXIES SERVICE] Iniciando alocação automática de Proxy para tenant: ${tenantId}`,
    );
    ...
    const proxyData = await provider.buyOrAllocateProxy(
      tenantId,
      currentProxies,
    );
    ...
    this.logger.log(
      `✅ [PROXIES SERVICE] Sucesso! Proxy ${newProxy.host}:${newProxy.port} (${newProxy.provider}) registrado para a conta.`,
    );
