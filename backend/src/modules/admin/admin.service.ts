...
import {
  getGlobalApiSettings,
  updateGlobalApiSettings,
...
export class AdminService {
  ...
  async getGlobalStats() {
    try {
      ...
      const campaignSum = await this.campaignRepo
  ...
  async createManualTenant(data: {
    ...
    if (existingTenant)
      throw new ConflictException('Email de empresa já cadastrado');
    ...
    try {
      ...
      throw new InternalServerErrorException(
        'Falha interna ao registrar tenant e usuário.',
      );
    } finally {
  ...
  async updateTenantStatus(
    ...
    return {
      success: true,
      message: `Tenant ${tenantId} status alterado para ${status}`,
    };
  }
  ...
  async updateTenantPlan(
    tenantId: string,
    payload: { planId: string; billingCycle?: string; trialEndsAt?: string },
  ) {
    const tenant = await this.tenantRepo.findOne({ where: { id: tenantId } });
    if (!tenant) throw new ConflictException('Cliente não encontrado');
    ...
    if (payload.trialEndsAt)
      updateObj.trialEndsAt = new Date(payload.trialEndsAt);
  ...
  async getRecentLogs() {
    ...
    return [
      ...
      },
      {
        ...
        level: 'warn',
        message: 'Tentativa de acesso inválida detectada.',
        context: 'Auth',
      },
    ];
  }
  ...
  async impersonateUser(userId: string) {
    ...
    if (!user) {
      throw new ConflictException(
        'Usuário para personificação não encontrado.',
      );
    }
