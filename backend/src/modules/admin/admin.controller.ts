...
export class AdminController {
  ...
  @ApiOperation({
    summary:
      'Cria um novo cliente/empresa manualmente com usuário admin inicial',
  })
  async createManualTenant(@Body() data: CreateManualTenantDto) {
    return this.adminService.createManualTenant(data);
  }

  @Patch('tenants/:id/status')
  @ApiOperation({
    summary: 'Atualiza o status (bloqueio/desbloqueio) de um cliente',
  })
  ...
  @Patch('tenants/:id')
  @ApiOperation({
    summary: 'Atualiza dados cadastrais e metadados flexíveis de um cliente',
  })
  ...
  async updateTenantPlan(
    @Param('id') id: string,
    @Body()
    body: { planId: string; billingCycle?: string; trialEndsAt?: string },
  ) {
  ...
  @ApiOperation({
    summary:
      '[Super Admin] Personifica um usuário cliente, retornando um novo token de acesso',
  })
