...
export class UploadsService {
  ...
  constructor(private configService: ConfigService) {
    ...
    if (enforcedBaseUrl) {
      this.baseUrl = enforcedBaseUrl.endsWith('/')
        ? enforcedBaseUrl.slice(0, -1)
        : enforcedBaseUrl;
    } else {
  ...
  async saveFile(
    ...
    if (file.mimetype.startsWith('image/')) {
      try {
        ...
        this.logger.log(
          `Image processed & metadata stripped with sharp: ${filename}`,
        );
      } catch (err: any) {
        this.logger.warn(
          `Failed to process image with sharp, saving original: ${err.message}`,
        );
    ...
    this.logger.log(
      `Saved file: ${filename} (${file.mimetype}, ${finalSize} bytes)`,
    );
