...
import {
  ...
  TimingMetadata,
} from '../anti-ban/human-behavior.service';
...
import {
  ...
  StackRoutingInput,
  StackType,
} from '../anti-ban/stack-router.service';
...
export class DispatcherProcessor extends WorkerHost {
  ...
  onModuleInit() {
    this.logger.log(
      `🔧 DispatcherProcessor initialized for queue: ${DISPATCH_QUEUE}`,
    );
  }
  ...
  async process(job: Job<DispatchJobData>): Promise<DispatchResult> {
    ...
    console.log(
      `🔥 [WORKER-DEBUG] Processing job ${job.id} for contact ${campaignContactId}`,
    );
    this.logger.debug(
      `Processing job ${job.id} for contact ${campaignContactId}`,
    );
    ...
    try {
      ...
      const tenantSettings = await this.campaignRepo.manager.query(
      ...
      if (tenant?.settings?.features?.services_enabled === false) {
        this.logger.warn(
          `Skipping contact ${campaignContactId}: Services disabled for tenant ${tenantId}`,
        );
      ...
      if (campaignContact.status === 'sent') {
        this.logger.log(
          `Skipping contact ${campaignContactId}: Already sent (Idempotency Check)`,
        );
      ...
      if (
        campaign.settings?.metaTemplateId &&
        campaign.settings?.wabaAccountId
      ) {
        // É campanha de WABA
        wabaAccount = await this.metaTemplatesService.getWabaAccount(
          tenantId,
          campaign.settings.wabaAccountId,
        );
        if (!wabaAccount) {
        ...
        instance = await this.selectInstance(
          tenantId,
          campaign?.instanceId,
      ...
      if (wabaAccount && campaign.settings?.metaTemplateId) {
        const [templateName, templateLanguage] =
          campaign.settings.metaTemplateId.split('|');
        const accessToken =
          await this.metaTemplatesService.getDecryptedAccessToken(
            wabaAccount.id,
          );
        const recipientPhone = contact.phone.replace(/\D/g, '');

        this.logger.log(
          `📤 Enviando template Meta ${templateName} para ${recipientPhone}`,
        );
        ...
        try {
          ...
          if (!metaTemplate) {
            this.logger.log(
              `🔍 Buscando definição do template "${templateName}" na Meta...`,
            );
            metaTemplate = await this.metaGraphApiService.getTemplate(
              wabaAccount.wabaId,
              accessToken,
              templateName,
            );
            if (metaTemplate) {
              ...
              throw new Error(
                `Definição do template "${templateName}" não encontrada na Meta. Verifique se o nome está correto.`,
              );
            }
          }

          this.logger.debug(
            `[TEMPLATE COMPONENTS]: ${JSON.stringify(metaTemplate?.components)}`,
          );
          ...
          if (!mediaId && campaign.settings?.metaMediaUrl) {
            try {
              const mediaComp = metaTemplate?.components?.find(
                (c: any) =>
                  c.type === 'HEADER' &&
                  ['IMAGE', 'DOCUMENT', 'VIDEO'].includes(c.format),
              );
              if (mediaComp) {
                this.logger.log(
                  `Subindo mídia (URL: ${campaign.settings.metaMediaUrl}) para os servidores da Meta...`,
                );
                mediaId = await this.metaGraphApiService.uploadMedia(
                  wabaAccount.phoneNumberId,
                  accessToken,
                  campaign.settings.metaMediaUrl,
                  mediaComp.format.toLowerCase(),
                );
              ...
              this.logger.error(
                `❌ Erro ao subir mídia: ${uploadError.message}`,
              );
            }
          }
            ...
            for (const comp of metaTemplate.components) {
              if (comp.type === 'BODY') {
                ...
                if (varCount > 0) {
                  componentsPayload.push({
                    ...
                    parameters: Array.from({ length: varCount }).map((_, i) => {
                      ...
                      const customVal =
                        campaign.settings?.metaVariables?.[varKey];
                      let finalVal = customVal;
                      if (!finalVal)
                        finalVal =
                          i === 0
                            ? contact.name?.trim() || 'Cliente'
                            : 'Detalhes';
                ...
                if (comp.format === 'TEXT' && comp.text?.includes('{{1}}')) {
                  const customVal =
                    campaign.settings?.metaVariables?.['header_1'];
                  componentsPayload.push({
                    type: 'header',
                    parameters: [
                      {
                        type: 'text',
                        text: customVal || contact.name?.trim() || 'Cliente',
                      },
                    ],
                  });
                } else if (
                  ...
                  if (mId) {
                    ...
                    if (
                      ...
                      mediaUrl.startsWith('http://192.168')
                    ) {
                      this.logger.error(
                        `❌ [META-WARNING] Tentando enviar URL local para Meta: ${mediaUrl}. Esta mensagem NÃO será entregue, pois a Meta não consegue acessar seu computador local.`,
                      );
                    }
          ...
          this.logger.log(
            `✅ Template enviado via WABA para ${recipientPhone} (ID: ${messageId})`,
          );
          ...
          if (messageId) {
            await this.inboxService
              ...
              .catch((err) =>
                this.logger.error(
                  `Failed to save WABA message to inbox: ${err.message}`,
                ),
              );
          }
          ...
          this.logger.error(
            `❌ Erro ao enviar WABA template: ${sendError.message}`,
          );
          throw sendError;
        }
      }
      ...
      if (campaign.flowId) {
        ...
        if (spacingDelayMs > 0) {
          this.logger.log(
            `⏳ Flow Campaign Spacing Delay: Instância ${instance.instanceName} ativa. Aguardando ${(spacingDelayMs / 1000).toFixed(1)}s antes de iniciar flow para ${contact.id} (Warmup Day: ${warmupDay})`,
          );
          await new Promise((resolve) => setTimeout(resolve, spacingDelayMs));
        }

        this.logger.log(
          `🌀 Executing Flow ${campaign.flowId} for contact ${contact.id}`,
        );
        ...
        try {
          ...
          await this.instanceRepo.increment(
            { id: instance.id },
            'dailySent',
            1,
          );
      ...
      const routingInput: StackRoutingInput = {
        ...
        messagesRemaining:
          (campaign.totalContacts || 0) - (campaign.sentCount || 0),
      ...
      this.logger.log(
        `⏳ Simulando comportamento humano para ${contact.phone}...`,
      );
      ...
      // 8. Enviar Mensagem
      this.logger.log(
        `📤 Enviando mensagem para ${contact.phone} via ${instance.instanceName}`,
      );
      ...
      try {
        ...
        console.log(
          `📡 [DISPATCHER-DEBUG] Provider response for ${contact.phone}:`,
          JSON.stringify(result),
        );
        this.logger.log(
          `✅ Mensagem enviada via ${instance.provider} para ${contact.phone} (ID: ${messageId})`,
        );
        ...
        if (messageId) {
          await this.inboxService
            ...
            .catch((err) =>
              this.logger.error(
                `Failed to save outbound message to inbox: ${err.message}`,
              ),
            );
        }
        ...
        this.logger.error(
          `❌ Erro ao enviar via ${instance.provider}: ${sendError.message}`,
        );
        ...
        if (isSenderMissing) {
          this.logger.warn(
            `🗑️ Instance ${instance.instanceName} returned "not found". Marking as disconnected in DB. Retrying contact with another chip...`,
          );
        ...
        if (msgLower.includes('404')) {
          this.logger.warn(
            `🚫 Destinatário ${contact.phone} falhou com 404 (número inválido ou sem WhatsApp).`,
          );
      ...
      this.logger.error(
        `❌ Failed to process ${campaignContactId}: ${error.message}`,
      );
      ...
      if (
        error.message === 'NO_AVAILABLE_INSTANCE' ||
        ...
        try {
          ...
          if (this.eventsGateway) {
            const camp = await this.campaignRepo.findOne({
              where: { id: campaignId },
          ...
          this.logger.error(
            `Falha ao pausar campanha ${campaignId}: ${pauseErr.message}`,
          );
        }
      ...
      if (error.message === 'SENDER_INSTANCE_NOT_FOUND') {
        this.logger.warn(
          `♻️ Remetente indisponível. Revertendo contato ${campaignContactId} para 'queued'.`,
        );
        await this.campaignContactRepo.update(campaignContactId, {
          status: 'queued',
  ...
  private async checkCampaignCompletion(campaignId: string) {
    try {
      ...
      if (processedCount >= campaign.totalContacts) {
        this.logger.log(
          `🏁 Campanha ${campaignId} concluída! (${processedCount}/${campaign.totalContacts})`,
        );
      ...
      this.logger.error(
        `Erro ao verificar conclusão da campanha: ${err.message}`,
      );
    }
  }
   ...
   */
  private getEffectiveLimit(instance: Instance): {
    limit: number;
    isWarmupLimit: boolean;
  } {
    if (
      instance.warmupEnabled &&
      instance.warmupDay !== undefined &&
      instance.warmupDay > 0 &&
      instance.warmupDay <= 14
    ) {
   ...
   */
  public handleWarmupLimitResponse(
    instanceId: string,
    action: 'override' | 'pause',
  ) {
    if (action === 'override') {
      this.warmupOverrides.set(instanceId, Date.now());
      this.logger.warn(
        `⚠️ User OVERRODE warmup limit for instance ${instanceId}. Sending will continue beyond warmup limit.`,
      );
    } else {
      this.warmupOverrides.delete(instanceId);
      this.logger.log(
        `✅ User confirmed warmup pause for instance ${instanceId}.`,
      );
    }
  }
  ...
  private async selectInstance(
    ...
    if (campaignInstanceIds && campaignInstanceIds.length > 0) {
      ...
      for (let attempt = 0; attempt < totalInstances; attempt++) {
        ...
        if (!instance) {
          this.logger.warn(
            `🔄 Round-Robin: Instance ${candidateId} not found in DB, skipping...`,
          );
          continue;
        }
        ...
        if (instance.status !== 'connected') {
          this.logger.warn(
            `🔄 Round-Robin: Instance ${instance.instanceName} not connected (status: ${instance.status}), skipping...`,
          );
          skippedInstances.push({
            name: instance.instanceName,
            reason: 'disconnected',
            sent: instance.dailySent || 0,
            limit: 0,
          });
          continue;
        }
        ...
        const { limit: effectiveLimit, isWarmupLimit } =
          this.getEffectiveLimit(instance);
          ...
          if (campaignId) {
            this.emitWarmupAlert(
              ...
              'limit_reached',
              {
                ...
                isWarmupLimit,
                message:
                  `Chip ${instance.instanceName} (${instance.phone}) atingiu o limite ` +
        ...
        if (usageRatio >= WARNING_THRESHOLD && isWarmupLimit && campaignId) {
          ...
          this.emitWarmupAlert(tenantId, instance, campaignId, 'warning', {
            ...
            message:
              `⚠️ Chip ${instance.instanceName} (${instance.phone}) está em ${Math.round(usageRatio * 100)}% ` +
      ...
      this.logger.error(
        `❌ No available instance found among ${totalInstances} configured instances for tenant ${tenantId}`,
      );
      ...
      if (campaignId && skippedInstances.length > 0) {
        ...
        this.emitWarmupAlert(
          ...
          {
            message:
              `🛑 TODOS os chips atingiram seus limites de warmup! ` +
        ...
        try {
          ...
          this.logger.warn(
            `⏸️ Campaign ${campaignId} AUTO-PAUSED: all chips exhausted warmup limits`,
          );
        } catch (err) {
    ...
    if (!instance) {
      this.logger.error(
        `Instance ${targetInstanceId} not found in DB for tenant ${tenantId}`,
      );
      return null;
    }
    ...
    if (instance.status !== 'connected') {
      this.logger.warn(
        `Instance ${instance.instanceName} is not connected (status: ${instance.status})`,
      );
      return null;
    }
    ...
    if (currentSent >= effectiveLimit) {
      ...
      if (campaignId) {
        ...
        try {
          ...
          this.logger.warn(
            `⏸️ Campaign ${campaignId} AUTO-PAUSED: single chip exhausted`,
          );
        } catch (err) {
          this.logger.error(`Failed to auto-pause campaign: ${err.message}`);
        }
  ...
  private mapStackToProvider(
    ...
    const idealProvider = stackToProvider[selectedStack];

    // Validar se o provider ideal está disponível no factory
    if (
      idealProvider &&
      this.whatsAppFactory.isProviderAvailable(idealProvider)
    ) {
  ...
  private async selectVariation(campaignId: string): Promise<MessageVariation> {
    ...
    const totalUse = variations.reduce(
      (sum, v) => sum + (v.useCount || 0) + 1,
      0,
    );
  ...
  async onCompleted(job: Job<DispatchJobData, DispatchResult>) {
    ...
    try {
      ...
      this.logger.error(
        `Erro ao buscar contato da campanha para WebSocket: ${err.message}`,
      );
    }
