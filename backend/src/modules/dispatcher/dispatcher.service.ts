...
export class DispatcherService {
  ...
  async enqueueCampaign(campaignId: string, tenantId: string): Promise<number> {
    ...
    if (!campaign) {
      throw new ForbiddenException(
        `Campaign ${campaignId} not found or access denied`,
      );
    }
    ...
    this.logger.log(
      `🚀 Adding ${jobs.length} jobs to queue: ${this.dispatchQueue.name}`,
    );
    try {
  ...
  async pauseCampaign(campaignId: string): Promise<void> {
    ...
    this.logger.log(
      `Campaign ${campaignId} paused, ${campaignJobs.length} jobs removed`,
    );
  }
