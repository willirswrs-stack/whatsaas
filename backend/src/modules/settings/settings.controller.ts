...
export class SettingsController {
  ...
  async getGlobalSettings(@CurrentUser() user: UserPayload) {
    if (user?.role !== 'super_admin') {
      throw new ForbiddenException(
        'Apenas Super Admins podem acessar configurações globais.',
      );
    }
  ...
  async updateGlobalSettings(
    ...
    if (user?.role !== 'super_admin') {
      throw new ForbiddenException(
        'Apenas Super Admins podem alterar configurações globais.',
      );
    }
  ...
  @Get('global/public')
  @ApiOperation({
    summary: 'Get public global settings (LLM info, warmup days)',
  })
