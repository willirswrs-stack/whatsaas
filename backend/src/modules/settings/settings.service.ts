...
const GLOBAL_DEFAULTS: GlobalConfig = {
  ...
  agentPromptMain:
    'Você é um assistente especialista em Marketing Digital e automação via WhatsApp. Ajude o usuário a configurar campanhas, criar fluxos e evitar banimentos.',
  agentPromptSpinner:
    'Reescreva o texto fornecido de {count} formas diferentes, mantendo o significado original mas variando vocabulário e estrutura. Retorne apenas as variações, separadas por "---".',
  agentPromptAntiban:
    'Você analisa mensagens de WhatsApp e reescreve de forma mais natural e humana, evitando padrões que podem ser detectados como spam. Mantenha o tom e a intenção original.',
  customLlmKey: '',
  customLlmUrl: '',
};
...
export class SettingsService {
  ...
  async getSettings(tenantId: string): Promise<Partial<TenantSettings>> {
    ...
    return {
      ...
      anthropicKey: settings.anthropicKey
        ? this.maskKey(settings.anthropicKey)
        : '',
      geminiKey: settings.geminiKey ? this.maskKey(settings.geminiKey) : '',
      groqKey: settings.groqKey ? this.maskKey(settings.groqKey) : '',
      elevenLabsKey: settings.extraSettings?.elevenLabsKey
        ? this.maskKey(settings.extraSettings.elevenLabsKey)
        : '',
      customLlmKey: settings.extraSettings?.customLlmKey
        ? this.maskKey(settings.extraSettings.customLlmKey)
        : '',
  ...
  async updateSettings(
    ...
    if (
      data.elevenLabsKey !== undefined ||
      data.customLlmKey !== undefined ||
      data.customLlmUrl !== undefined
    ) {
      if (!settings.extraSettings) settings.extraSettings = {};
      if (
        data.elevenLabsKey !== undefined &&
        !data.elevenLabsKey.includes('*')
      ) {
  ...
  async getGlobalSettings(): Promise<GlobalConfig> {
    let systemSettings = await this.settingsRepo.findOne({
      where: { tenantId: SYSTEM_TENANT_ID },
    });

    if (!systemSettings) {
      systemSettings = this.settingsRepo.create({
    ...
    return {
      ...
      anthropicKey: config.anthropicKey
        ? this.maskKey(config.anthropicKey)
        : '',
      ...
      elevenLabsKey: config.elevenLabsKey
        ? this.maskKey(config.elevenLabsKey)
        : '',
      customLlmKey: config.customLlmKey
        ? this.maskKey(config.customLlmKey)
        : '',
  ...
  async updateGlobalSettings(data: GlobalConfig): Promise<void> {
    ...
    const updated: Record<string, any> = {
      ...
      globalLlmTemperature:
        data.globalLlmTemperature ?? existing.globalLlmTemperature,
      globalLlmMaxTokens:
        data.globalLlmMaxTokens ?? existing.globalLlmMaxTokens,
      warmupDaysColdOutbound:
        data.warmupDaysColdOutbound ?? existing.warmupDaysColdOutbound,
      warmupDaysWarmOutbound:
        data.warmupDaysWarmOutbound ?? existing.warmupDaysWarmOutbound,
      ...
      agentPromptSpinner:
        data.agentPromptSpinner ?? existing.agentPromptSpinner,
      agentPromptAntiban:
        data.agentPromptAntiban ?? existing.agentPromptAntiban,
    };
  ...
  async getEffectiveLLMKeys(tenantId: string): Promise<LLMApiKeys> {
    ...
    const systemSettings = await this.settingsRepo.findOne({
      where: { tenantId: SYSTEM_TENANT_ID },
    });
