...
export class AsaasService {
  ...
  async createCustomer(data: {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          result.errors?.[0]?.description || 'Falha ao criar cliente no Asaas',
        );
      }
  ...
  async createSubscription(data: {
    ...
    try {
      this.logger.log(
        `Criando assinatura Asaas p/ Customer: ${data.customerId}`,
      );
      ...
      if (!response.ok) {
        throw new Error(
          result.errors?.[0]?.description || 'Falha ao criar assinatura',
        );
      }
  ...
  async getSubscription(subscriptionId: string) {
    try {
      ...
      if (!response.ok)
        throw new Error(
          result.errors?.[0]?.description || 'Falha ao buscar assinatura',
        );
      return result;
    } catch (e) {
  ...
  async updateSubscription(
    ...
    data: { value: number; description: string; cycle?: string },
  ) {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          result.errors?.[0]?.description || 'Falha ao atualizar assinatura',
        );
      }
      return result;
    } catch (e) {
