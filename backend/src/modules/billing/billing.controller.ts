...
export class BillingController {
    ...
    if (tenant?.asaasSubscriptionId) {
      try {
        if (tenant?.status !== 'active') {
          const paymentInfo =
            await this.asaasService.getSubscriptionPaymentInfo(
          ...
          if (
            ...
            this.logger.log(
              `Tenant ${tenant.name} ativado automaticamente via consulta no GET /status`,
            );
          }
        }
        ...
        this.logger.error(
          `Erro ao consultar status da assinatura: ${e.message}`,
        );
      }
    }
  ...
  async createSubscription(
    @Request() req,
    @Body() body: { planId: string; cpfCnpj: string; phone?: string },
  ) {
    const user = req.user;
    const tenantId = user.tenantId;

    this.logger.log(
      `Iniciando checkout para tenant ${tenantId} - Plano ${body.planId}`,
    );
    ...
    if (tenant.asaasSubscriptionId) {
      this.logger.log(
        `Atualizando assinatura existente ${tenant.asaasSubscriptionId} para novo plano`,
      );
      subRes = await this.asaasService.updateSubscription(
        tenant.asaasSubscriptionId,
        {
    ...
    const paymentInfo = await this.asaasService.getSubscriptionPaymentInfo(
      subRes.id,
    );
  ...
  async handleAsaasWebhook(@Body() payload: any) {
    ...
    if (eventType === 'PAYMENT_RECEIVED' || eventType === 'PAYMENT_CONFIRMED') {
      ...
      this.logger.log(
        `💰 Pagamento confirmado para cliente ${customerId}! Ativando acesso...`,
      );
      ...
      if (tenant) {
        ...
        await this.tenantRepo.save(tenant);
        this.logger.log(
          `✅ Tenant ${tenant.name} foi ATIVADO com sucesso via Webhook!`,
        );
      }
    }
    ...
    if (eventType === 'PAYMENT_OVERDUE') {
      ...
      if (tenant) {
        ...
        await this.tenantRepo.save(tenant);
        this.logger.warn(
          `⛔ Tenant ${tenant.name} SUSPENSO por falta de pagamento.`,
        );
      }
    }
