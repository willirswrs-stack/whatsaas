...
describe('CampaignsController', () => {
  ...
  beforeEach(async () => {
    campaignsService = {
      ...
      generateVariations: jest
        .fn()
        .mockResolvedValue({ variations: [], tokensUsed: 100 }),
      start: jest
        .fn()
        .mockResolvedValue({ ...mockCampaign, status: 'running' }),
      pause: jest.fn().mockResolvedValue({ ...mockCampaign, status: 'paused' }),
      resume: jest
        .fn()
        .mockResolvedValue({ campaign: mockCampaign, queuedMessages: 5 }),
      cancel: jest
        .fn()
        .mockResolvedValue({ ...mockCampaign, status: 'cancelled' }),
      delete: jest.fn().mockResolvedValue({ deleted: true }),
      getStats: jest
        .fn()
        .mockResolvedValue({ total: 10, sent: 5, delivered: 3, failed: 0 }),
      findAllTemplates: jest
        .fn()
        .mockResolvedValue([{ id: 'template-123', name: 'Test' }]),
      createTemplate: jest
        .fn()
        .mockResolvedValue({ id: 'template-123', name: 'New Template' }),
      findAllContacts: jest
        .fn()
        .mockResolvedValue([{ id: 'contact-123', phone: '5511999999999' }]),
      createContact: jest
        .fn()
        .mockResolvedValue({ id: 'contact-123', phone: '5511999999999' }),
  ...
  describe('generateVariations', () => {
    it('should generate AI variations', async () => {
      ...
      const result = await controller.generateVariations(
        'campaign-123',
        options,
        'tenant-123',
      );
  ...
  describe('importContacts', () => {
    ...
    it('should throw error if no file uploaded', async () => {
      await expect(
        controller.importContacts(null as any, 'tenant-123'),
    ...
    it('should throw error if CSV has no phone column', async () => {
      ...
      await expect(
        controller.importContacts(file, 'tenant-123'),
