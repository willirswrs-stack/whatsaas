...
export class CampaignsService {
  ...
  async create(
    ...
    const finalContactIds = new Set(providedIds);
    ...
    const campaign = this.campaignRepo.create({
      ...
      instanceIds:
        (campaignData.instanceIds?.length || 0) > 0
  ...
  async update(
    ...
    if (campaign.status === 'completed' || campaign.status === 'cancelled') {
      throw new BadRequestException(
        'Campanhas finalizadas ou canceladas não podem mais ser editadas.',
      );
    }
  ...
  async start(id: string, tenantId: string) {
    ...
    if (contactCount === 0) {
      throw new Error(
        'Campanha não tem contatos. Adicione contatos antes de iniciar.',
      );
    }
    ...
    if (campaign.flowId) {
      const flow = await this.flowRepo.findOne({
        where: { id: campaign.flowId },
      });
      if (!flow) {
        throw new BadRequestException(
          `Fluxo ${campaign.flowId} não encontrado.`,
        );
      }
      if (flow.status !== 'active') {
        this.logger.log(
          `🔄 Auto-ativando fluxo "${flow.name}" (status era: ${flow.status}) para campanha ${id}`,
        );
    ...
    const instanceIds =
      campaign.instanceIds?.length > 0
        ? campaign.instanceIds
        : campaign.instanceId
          ? [campaign.instanceId]
          : [];
    if (instanceIds.length > 0) {
      const instances = await this.campaignRepo.manager
      ...
      this.logger.log(
        `✅ ${connectedInstances.length}/${instanceIds.length} instâncias conectadas para campanha ${id}`,
      );
    }
    ...
    if (
      ...
      if (campaign.aiSpinEnabled && campaign.variationCount > 1) {
        try {
          this.logger.log(
            `🤖 Gerando ${campaign.variationCount} variações via IA para campanha ${id}...`,
          );
          ...
          const variations = result.variations.map((content, index) =>
            this.variationRepo.create({
              ...
              variationIndex: index,
              content,
              contentHash: Buffer.from(content)
                .toString('base64')
                .substring(0, 32),
            }),
          );
          await this.variationRepo.save(variations);
          this.logger.log(
            `✅ ${variations.length} variações criadas via IA (${result.tokensUsed} tokens)`,
          );
          ...
          this.logger.warn(
            `⚠️ Erro ao gerar variações IA: ${err.message}, usando padrão`,
          );
          ...
          const defaultVariation = this.variationRepo.create({
            ...
            variationIndex: 0,
            content: baseContent,
            contentHash: Buffer.from(baseContent)
              .toString('base64')
              .substring(0, 32),
          });
    ...
    try {
      const enqueued = await this.dispatcherService.enqueueCampaign(
        id,
        tenantId,
      );
      this.logger.log(
        `📤 ${enqueued} mensagens enfileiradas para campanha ${id}`,
      );
    } catch (err: any) {
  ...
  async schedule(id: string, tenantId: string, scheduledAtStr: string) {
    ...
    if (contactCount === 0) {
      throw new BadRequestException(
        'Campanha não tem contatos. Adicione contatos antes de agendar.',
      );
    }
    ...
    this.logger.log(
      `⏰ Campanha ${id} agendada para ${scheduledAt.toISOString()} (em ${Math.round(delay / 1000)}s)`,
    );
  ...
  async resume(id: string, tenantId: string) {
    await this.findOne(id, tenantId); // Verify ownership
    const queuedCount = await this.dispatcherService.resumeCampaign(
      id,
      tenantId,
    );
    return {
  ...
  async duplicate(id: string, tenantId: string) {
    ...
    this.logger.log(
      `📋 Campaign ${id} duplicated as ${savedCampaign.id} with ${originalContacts.length} contacts`,
    );
  ...
  async splitIntoBatches(
    ...
    if (batchSize < 10) {
      throw new BadRequestException(
        'O tamanho do batch deve ser no mínimo 10 contatos.',
      );
    }
    ...
    for (let i = 0; i < allContacts.length; i += batchSize) {
      chunks.push(
        allContacts.slice(i, i + batchSize).map((cc) => cc.contactId),
      );
    }
    ...
    if (scheduleOptions?.firstBatchAt) {
      ...
      if (isNaN(firstBatchDate.getTime())) {
        throw new BadRequestException(
          'Data de agendamento inválida para o primeiro batch.',
        );
      }
      if (firstBatchDate <= new Date()) {
        throw new BadRequestException(
          'A data do primeiro batch deve estar no futuro.',
        );
      }
    }
    ...
    for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
      ...
      if (firstBatchDate && scheduleOptions) {
        ...
        scheduledAt.setTime(
          scheduledAt.getTime() +
            batchIndex * (scheduleOptions.intervalHours || 24) * 3600 * 1000,
        );
      }
  ...
  async findCampaignContacts(
    ...
    const qb = this.campaignContactRepo
  ...
  async retryFailed(id: string, tenantId: string) {
    ...
    this.logger.log(
      `🌀 Resetting ${result.affected} failed contacts for campaign ${id}`,
    );

    // 2. Atualizar status da campanha se necessário
    // Decrementar contagem de falhas (aproximado)
    await this.campaignRepo.decrement(
      { id },
      'failedCount',
      result.affected || 0,
    );
  ...
  async getEstimate(id: string, tenantId: string) {
    ...
    for (const instId of instanceIds) {
      const inst = await this.campaignRepo.manager
        .getRepository(Instance)
        .findOne({
          where: { id: instId },
        });
    ...
    const avgDelayPerMessageMs = Math.round(
      avgBaseDelayMs * avgWarmupMultiplier,
    );
    ...
    const totalAvgDelayPerMsg =
      avgDelayPerMessageMs + avgTypingMs + avgPreTypingMs;

    // Se flow campaign, delay é mais simples (sem typing simulation do dispatcher)
    const effectiveDelayPerMsg = campaign.flowId
      ? avgDelayPerMessageMs
      : totalAvgDelayPerMsg;
    ...
    const activeMsPerDay = Math.max(
      activeMinutesPerDay * 60 * 1000,
      8 * 3600 * 1000,
    ); // min 8h
    ...
    const effectiveDelayBetweenSends =
      effectiveDelayPerMsg / effectiveConcurrency;
    ...
    const msRemainingToday = Math.max(
      0,
      todayEndActive.getTime() - now.getTime(),
    );

    // Verificar limite diário
    const messagesCanSendToday =
      connectedCount > 0
        ? Math.min(
            totalRemainingToday,
            Math.floor(msRemainingToday / effectiveDelayBetweenSends),
          )
        : 0;
    ...
    if (!spansMultipleDays) {
      ...
      const messagesPerDay =
        connectedCount > 0
          ? Math.min(
              totalRemainingToday || Infinity,
              Math.floor(activeMsPerDay / effectiveDelayBetweenSends),
            )
          : 1; // Avoid division by zero
      const additionalDays = Math.ceil(
        remainingAfterToday / Math.max(messagesPerDay, 1),
      );

      estimatedFinishAt = new Date(now);
      estimatedFinishAt.setDate(estimatedFinishAt.getDate() + additionalDays);
      estimatedFinishAt.setHours(
        startH +
          Math.floor(
            ((remainingAfterToday % messagesPerDay) *
              effectiveDelayBetweenSends) /
              3600000,
          ),
        startM,
        0,
