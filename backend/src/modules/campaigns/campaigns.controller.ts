...
export class CampaignsController {
  ...
  async getGlobalContactStats(@CurrentTenant() tenantId: string) {
    ...
    const list = await this.campaignsService.findAllContacts(tenantId, {
      limit: 5,
    } as any);
    return {
      stats,
  ...
  async importContacts(
    ...
    if (phoneIndex === -1) {
      throw new BadRequestException(
        'CSV must have a "telefone" or "phone" column',
      );
    }
    ...
    // Import to database
    const imported = await this.campaignsService.importContacts(
      tenantId,
      contacts,
    );
  ...
  @ApiOperation({
    summary: 'Respond to warmup limit alert (override or pause)',
  })
  async warmupOverride(
    ...
    if (!body.instanceId || !['override', 'pause'].includes(body.action)) {
      throw new BadRequestException(
        'instanceId and action (override|pause) are required',
      );
    }
    this.dispatcherProcessor.handleWarmupLimitResponse(
      body.instanceId,
      body.action,
    );
    return {
  ...
  async start(@Param('id') id: string, @CurrentTenant() tenantId: string) {
    return this.campaignsService.start(id, tenantId);
  }

  @Post(':id/pause')
  @ApiOperation({ summary: 'Pause campaign' })
  async pause(@Param('id') id: string, @CurrentTenant() tenantId: string) {
    return this.campaignsService.pause(id, tenantId);
  }

  @Post(':id/resume')
  @ApiOperation({ summary: 'Resume paused campaign' })
  async resume(@Param('id') id: string, @CurrentTenant() tenantId: string) {
  ...
  async duplicate(@Param('id') id: string, @CurrentTenant() tenantId: string) {
  ...
  async split(
    ...
    if (!body.batchSize || body.batchSize < 10) {
      throw new BadRequestException(
        'batchSize deve ser um número maior ou igual a 10.',
      );
    }
    return this.campaignsService.splitIntoBatches(
      ...
      body.firstBatchAt
        ? {
            firstBatchAt: body.firstBatchAt,
  ...
  async getStats(@Param('id') id: string, @CurrentTenant() tenantId: string) {
