...
export class ContactsService {
  ...
  async findAllContacts(tenantId: string, query: ContactQueryDto) {
    this.logger.log(
      `findAllContacts: tenantId=${tenantId} query=${JSON.stringify(query)}`,
    );
    ...
    this.logger.log(
      `findAllContacts: page=${page} limit=${limit} skip=${skip} where=${JSON.stringify(where)}`,
    );
    ...
    if (!search && (!tagIds || tagIds.length === 0)) {
      ...
      this.logger.log(
        `Repository findAndCount returned: contacts=${contacts.length}, total=${total}`,
      );
    } else {
      ...
      if (query.category) {
        qb.andWhere('contact.category ILIKE :category', {
      ...
      qb.orderBy(`contact.${sortByStr}`, sortOrderStr).skip(skip).take(limit);
      ...
      this.logger.log(
        `QueryBuilder returned: contacts=${contacts.length}, total=${total}`,
      );
    }
  ...
  async bulkDeleteContactsByTags(tenantId: string, tagIds: string[]) {
    if (!tagIds || tagIds.length === 0) {
      throw new BadRequestException(
        'Nenhuma tag fornecida para exclusão em massa',
      );
    }
    ...
    const uniqueContactIds = [
      ...new Set(contactTags.map((ct) => ct.contactId)),
    ];
    ...
    if (idsToDelete.length === 0) {
      return {
        message:
          'Nenhum contato encontrado com as tags informadas neste tenant',
      };
    }
  ...
  async importContacts(tenantId: string, contacts: CreateContactDto[]) {
      ...
      for (let i = 0; i < newContacts.length; i += chunkSize) {
        ...
        try {
          ...
          savedChunk.forEach((savedContact, idx) => {
            ...
            const tagReq = contactTagsToInsert.find(
              (t) => t.contactIndex === originalIndex,
            );
            if (tagReq) {
          ...
          results.errors.push(
            `Erro ao salvar lote ${i / chunkSize}: ${err.message}`,
          );
    ...
    const existingContactTags =
      contactIdsToCheck.length > 0
        ? await this.contactTagRepository.find({
    ...
    for (const req of contactTagsToInsert) {
      ...
      for (const tagId of req.tagIds) {
        ...
        if (!existingTagsForContact.includes(tagId)) {
          finalTagsToInsert.push(
            this.contactTagRepository.create({
    ...
    if (finalTagsToInsert.length > 0) {
      ...
      for (let i = 0; i < finalTagsToInsert.length; i += chunkSize) {
        await this.contactTagRepository.save(
          finalTagsToInsert.slice(i, i + chunkSize),
        );
      }
    }
  ...
  async importFromWhatsApp(
    ...
    this.logger.log(
      `Importing contacts from WhatsApp instance: ${instance.instanceName} for tenant: ${tenantId}`,
    );
    ...
    if (wahaContacts && wahaContacts.length > 0) {
      for (const contact of wahaContacts) {
        ...
        const rawId =
          contact.remoteJid || contact.id?._serialized || contact.id || '';
        ...
        const name =
          contact.name || contact.pushName || contact.notify || undefined;
    ...
    if (includeGroups && typeof provider.getGroupParticipants === 'function') {
      this.logger.log(
        `Fetching group participants for instance: ${instance.instanceName}`,
      );
      const groupParticipants = await provider.getGroupParticipants(
      ...
      for (const participant of groupParticipants) {
        ...
        if (
          !phoneStr ||
          (!phoneStr.includes('@s.whatsapp.net') && !phoneStr.includes('@lid'))
        )
          continue;

        const phone = phoneStr.includes('@lid') ? phoneStr : phoneStr.split('@')[0];
        if (!phone) continue;
        ...
        const existingIndex = contactsToImport.findIndex(
          (c) => c.phone === phone,
        );
        if (existingIndex >= 0) {
          ...
          if (
            tagId &&
            !contactsToImport[existingIndex].tagIds.includes(tagId)
          ) {
          ...
          contactsToImport.push({
            ...
            tagIds: allTags,
            customFields: participant.username
    ...
    this.logger.log(
      `Found ${contactsToImport.length} valid contacts to import from WhatsApp`,
    );
  ...
  async parseAndImportHeaderFile(
    tenantId: string,
    buffer: Buffer,
    mimetype: string,
  ) {
  ...
  async verifyContacts(
    ...
    instanceName: string,
    contactIds: string[],
    providerTypeStr: 'evolution' | 'waha',
  ) {
    ...
    for (const contactId of contactIds) {
      ...
      try {
        const isOnWhatsapp = await provider.isOnWhatsApp(
          instanceName,
          contact.phone,
        );
        ...
        this.logger.error(
          `Error verifying contact ${contactId}: ${error.message}`,
        );
  ...
  async getContactStats(tenantId: string) {
    ...
    const valid = await this.contactRepository.count({
      where: { tenantId, isValid: true },
    });
    const optedOut = await this.contactRepository.count({
      where: { tenantId, optedOut: true },
    });
    const onWhatsapp = await this.contactRepository.count({
      where: { tenantId, onWhatsapp: true },
    });
