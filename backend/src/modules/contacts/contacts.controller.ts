...
export class ContactsController {
  ...
  async findAll(@Request() req, @Query() query: ContactQueryDto) {
    ...
    const result = await this.contactsService.findAllContacts(
      req.user.tenantId,
      query,
    );
  ...
  async importFromWhatsApp(
    ...
    return this.contactsService.importFromWhatsApp(
      req.user.tenantId,
      instanceId,
      includeGroups,
      tagIds,
        ...
        if (
          ...
          callback(
            new BadRequestException(
              'Apenas arquivos .csv, .xls ou .xlsx são permitidos',
            ),
            false,
          );
        }
  ...
  async importContactsFile(
    ...
    return this.contactsService.parseAndImportHeaderFile(
      req.user.tenantId,
      file.buffer,
      file.mimetype,
    );
  ...
  async bulkDeleteByTags(@Request() req, @Body('tagIds') tagIds: string[]) {
    return this.contactsService.bulkDeleteContactsByTags(
      req.user.tenantId,
      tagIds,
    );
  }
