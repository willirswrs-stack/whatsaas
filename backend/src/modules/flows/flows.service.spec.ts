...
describe('FlowsService', () => {
  ...
  describe('findById', () => {
    ...
    it('should throw NotFoundException if not found', async () => {
      ...
      await expect(
        service.findById('tenant-123', 'non-existent'),
  ...
  describe('create', () => {
    it('should create flow with default start node', async () => {
      flowRepo.create!.mockImplementation((data) => ({
        ...mockFlow(),
        ...data,
      }));
  ...
  describe('update', () => {
    it('should update an existing flow', async () => {
      ...
      expect(flowRepo.update).toHaveBeenCalledWith('flow-123', {
        name: 'Updated Flow',
      });
    });
  });
  ...
  describe('activate', () => {
    ...
    it('should throw if flow has no nodes', async () => {
      ...
      await expect(service.activate('tenant-123', 'flow-123')).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe('Triggers', () => {
    describe('createTrigger', () => {
      it('should create a trigger for a flow', async () => {
        ...
        flowRepo.findOne!.mockResolvedValue(flow);
        triggerRepo.find!.mockResolvedValue([]);
        triggerRepo.create!.mockImplementation((data) => ({
          ...mockTrigger(),
          ...data,
        }));
    ...
    describe('updateTrigger', () => {
      ...
      it('should throw if trigger not found', async () => {
        ...
        await expect(
          service.updateTrigger('tenant-123', 'non-existent', {
            active: false,
          }),
        ).rejects.toThrow(NotFoundException);
      });
  ...
  describe('Executions', () => {
    describe('startExecution', () => {
      it('should start a flow execution', async () => {
        ...
        flowRepo.findOne!.mockResolvedValue(flow);
        triggerRepo.find!.mockResolvedValue([]);
        executionRepo.create!.mockImplementation((data) => ({
          ...mockExecution(),
          ...data,
        }));
