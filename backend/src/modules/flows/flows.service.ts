...
export class FlowsService implements OnModuleInit {
  ...
  async recoverDelayedExecutions() {
    try {
      ...
      if (delayedExecutions.length > 0) {
        console.log(
          `[Flow] Recovering ${delayedExecutions.length} delayed executions`,
        );
        delayedExecutions.forEach((exec) => {
          // FIX: Call resumeExecution (resets status + nextActionAt) instead of processExecution
          this.resumeExecution(exec.id).catch((err) =>
            console.error(`Error recovering execution ${exec.id}:`, err),
          );
        });
      }
    } catch (error) {
  ...
  async startExecution(tenantId: string, dto: ExecuteFlowDto) {
    ...
    if (flow.status !== 'active') {
      throw new BadRequestException(
        'O fluxo precisa estar ativo para ser executado',
      );
    }
    ...
    if (hoursConfig?.enabled && !dto.initialVariables?.isTest) {
      ...
      if (!isInside) {
        console.log(
          `[Flow] Execution blocked for ${flow.name}: Outside business hours.`,
        );
    ...
    this.processExecution(saved.id).catch((err) =>
  ...
  private findNextEdge(
    ...
    if (preferredHandle) {
      const handleEdge = outEdges.find(
        (e) => e.sourceHandle === preferredHandle,
      );
    ...
    console.warn(
      `[Flow] Multiple edges from node ${currentNodeId} with no default — using first edge. Edges: ${JSON.stringify(outEdges.map((e) => ({ id: e.id, sourceHandle: e.sourceHandle, target: e.target })))}`,
    );
    return outEdges[0];
  }
  ...
  async updateCampaignContactMessageId(execution: any, messageId: string) {
    ...
    if (campaignContactId && messageId) {
      try {
        ...
        console.error(
          `[Flow] Error updating campaign contact ${campaignContactId}:`,
          err.message,
        );
      }
    }
  ...
  async markCampaignContactAsSent(execution: any) {
    ...
    if (campaignContactId) {
      try {
        ...
        console.error(
          `[Flow] Error updating campaign contact ${campaignContactId} to sent:`,
          err.message,
        );
      }
    }
  ...
  private async checkCampaignCompletion(campaignId: string) {
    try {
      const campaignRepo =
        this.campaignContactRepository.manager.getRepository(Campaign);
      ...
      if (processedCount >= campaign.totalContacts) {
        console.log(
          `[Flow] 🏁 Campanha ${campaignId} concluída! (${processedCount}/${campaign.totalContacts})`,
        );
      ...
      console.error(
        `[Flow] Erro ao verificar conclusão da campanha ${campaignId}:`,
        err.message,
      );
    }
  }
    ...
    try {
      ...
      if (isLocalhost || isPrivateIp) {
        ...
        if (parsed.port === String(port) || (!parsed.port && port === 80)) {
          ...
          if (isProd) {
            const uploadsBase =
              this.configService.get<string>('UPLOADS_BASE_URL');
            if (uploadsBase) {
              try {
  ...
  async processExecution(executionId: string, processCurrentNode = false) {
    ...
    if (!execution || execution.status !== 'running') {
      if (execution)
        console.log(
          `[Flow] Execution ${executionId.slice(0, 8)} skipped (status: ${execution.status})`,
        );
      return;
    }
    ...
    if (execution.nextActionAt) {
      ...
      if (remaining > 500) {
        console.log(
          `[Flow] ⏳ Execution ${executionId.slice(0, 8)} still delayed for ${Math.ceil(remaining / 1000)}s — skipping (BullMQ will resume)`,
        );
    ...
    if (processCurrentNode) {
      nextNode = flow.nodes.find((n) => n.id === currentNodeId);
    } else {
      edge = this.findNextEdge(flow.edges, currentNodeId);
      if (!edge) {
        console.log(
          `[Flow] End of flow reached for execution ${executionId} (no outgoing edges from ${currentNodeId})`,
        );
      ...
      nextNode = flow.nodes.find((n) => n.id === edge.target);
    }

    if (!nextNode) {
      console.error(
        `[Flow] Next node not found for execution ${executionId} (currentNodeId: ${currentNodeId})`,
      );
      execution.status = 'completed';
      execution.completedAt = new Date();
      await this.executionRepository.save(execution);
    ...
    const edgeId = edge ? edge.id : 'direct';
    const sourceHandle = edge ? edge.sourceHandle || 'default' : 'direct';
    console.log(
      `[Flow] [${executionId.slice(0, 8)}] ▶ Processing node: ${nextNode.id} | type: ${nodeType} | label: ${nextNode.data?.label || '-'} | from edge: ${edgeId} (handle: ${sourceHandle})`,
    );
    ...
    try {
      ...
      if (aiNodeTypes.includes(nodeType)) {
        ...
        if (instance?.status === 'connected' && contact) {
          ...
          // PREVENÇÃO ATIVA
          const timing =
            this.humanBehavior.generateTimingMetadata(responseContent);
          ...
          if (messageId) {
            ...
            await this.inboxService
              ...
              .catch((err) =>
                console.error(
                  `[Flow] Error saving AI message to inbox:`,
                  err.message,
                ),
              );
          }
          ...
          execution.logs.push({
            ...
            data: {
              ...
              variationStyle: randomStyle.substring(0, 50),
              seed: randomSeed,
            },
          });
          console.log(
            `[Flow] 🎯 AI variation applied: style="${randomStyle.substring(0, 40)}..." seed=${randomSeed}`,
          );
        } else {
          console.warn(
            `[Flow] Instance not connected or contact missing for AI node ${nextNode.id}`,
          );
        }
      }
      ...
      // ==================== MESSAGE NODE ====================
        ...
        if (instance?.status === 'connected' && contact) {
          ...
          const nodeData = nextNode.data;
          const content =
            nodeData?.config?.message || nodeData?.content || 'Olá';
          ...
          if (messageId) {
            ...
            await this.inboxService
              ...
              .catch((err) =>
                console.error(
                  `[Flow] Error saving text message to inbox:`,
                  err.message,
                ),
              );
          }
          ...
          console.warn(
            `[Flow] Instance not connected or contact missing for Message node ${nextNode.id}`,
          );
        }
      }
      ...
      // ==================== MEDIA NODES (video, image, audio, document, sticker) ====================
        ...
        ['video', 'media', 'image', 'audio', 'document', 'sticker'].includes(
          nodeType,
        )
      ) {
        const { instance, contact } = await this.getExecutionContext(execution);
        ...
        if (instance?.status === 'connected' && contact) {
          ...
          const nodeData = nextNode.data;
          const rawMediaUrl =
            nodeData?.config?.mediaUrl ||
            nodeData?.config?.url ||
            nodeData?.config?.file;
          const mediaUrl = this.resolveMediaUrl(rawMediaUrl);
          const caption =
            nodeData?.config?.caption || nodeData?.config?.message || '';
          const mediaType =
            nodeData?.config?.mediaType ||
            (nodeType === 'sticker' ? 'sticker' : nodeType);
          ...
          if (mediaUrl) {
            try {
              ...
              if (messageId) {
                ...
                await this.inboxService
                  ...
                  .catch((err) =>
                    console.error(
                      `[Flow] Error saving media message to inbox:`,
                      err.message,
                    ),
                  );
              }
              ...
              console.error(
                `[Flow] Media Send Error at node ${nextNode.id}:`,
                mediaErr.message,
              );
              execution.logs.push({
                nodeId: nextNode.id,
            ...
            console.warn(
              `[Flow] No media URL configured for node ${nextNode.id}`,
            );
            execution.logs.push({
              nodeId: nextNode.id,
          ...
          console.warn(
            `[Flow] Instance not connected for Media node ${nextNode.id}`,
          );
        }
      }
      ...
      // ==================== LINK NODE ====================
        ...
        if (instance?.status === 'connected' && contact) {
          ...
          if (url) {
            ...
            if (res?.messageId) {
              await this.updateCampaignContactMessageId(
                execution,
                res.messageId,
              );
              await this.inboxService
                ...
                .catch((err) =>
                  console.error(
                    `[Flow] Error saving link message to inbox:`,
                    err.message,
                  ),
                );
            }
      ...
      // ==================== BUTTONS NODES (buttonsDefault, buttonsCopy, buttonsActions) ====================
        ...
        ['buttonsDefault', 'buttonsCopy', 'buttonsActions', 'buttons'].includes(
          nodeType,
        )
      ) {
        const { instance, contact } = await this.getExecutionContext(execution);
        ...
        if (instance?.status === 'connected' && contact) {
          ...
          try {
            ...
            if (
              (instance.provider as string) === 'evolution_cloud' &&
              (provider as any).sendButtons
            ) {
              const res = await (provider as any).sendButtons(
                instance.instanceName,
                contact.phone,
                {
                  text: message,
                  buttons: buttons.map((b: any) => ({
                    id:
                      b.id || `btn-${Math.random().toString(36).substr(2, 9)}`,
            ...
            console.error(
              `[Flow] Error sending buttons at node ${nextNode.id}, falling back to text:`,
              btnErr.message,
            );
            if (buttons.length > 0) {
              const buttonList = buttons
                .map(
                  (b: any, i: number) => `${i + 1}. ${b.label || b.text || b}`,
                )
                .join('\n');
              finalContent = `${message}\n\n${buttonList}`;
            }
          ...
          if (finalMessageId) {
            ...
            await this.inboxService
              ...
              .catch((err) =>
                console.error(
                  `[Flow] Error saving buttons message to inbox:`,
                  err.message,
                ),
              );
          }
          ...
          console.warn(
            `[Flow] Instance not connected for Buttons node ${nextNode.id}`,
          );
        }
      }
      ...
      // ==================== EMAIL NODE ====================
        ...
        const nodeData = nextNode.data;
        console.log(
          `[Flow] Email node ${nextNode.id} - mock (not implemented yet)`,
        );
        execution.logs.push({
          nodeId: nextNode.id,
          action: 'email_mock',
        ...
        if (instance?.status === 'connected' && contact) {
          ...
          if (res?.messageId) {
            await this.inboxService
              ...
              .catch((err) =>
                console.error(
                  `[Flow] Error saving question to inbox:`,
                  err.message,
                ),
              );
          }
      ...
      // ==================== DELAY NODE ====================
        ...
        const nodeData = nextNode.data;
        const rawSeconds =
          nodeData?.config?.seconds ||
          nodeData?.config?.delay ||
          nodeData?.config?.time ||
          '5';
        const delaySeconds = Math.max(
          1,
          Math.min(86400, parseInt(String(rawSeconds), 10) || 5),
        ); // Increase max to 24h
        const delayMs = delaySeconds * 1000;

        console.log(
          `[Flow] ⏳ Delay Node ${nextNode.id}: scheduling resume in ${delaySeconds}s (${delayMs}ms)...`,
        );
      ...
      // ==================== CONDITION NODE ====================
        ...
        console.log(
          `[Flow] Condition node ${nextNode.id}: field=${field}, op=${operator}, expected=${value}, actual=${actualValue}, met=${conditionMet}`,
        );
        ...
        if (condEdge) {
          ...
          if (condNextNode) {
            ...
            await this.processExecution(executionId, true);
          }
        } else {
          console.log(
            `[Flow] No edge for condition result '${handleId}' at node ${nextNode.id}. Ending.`,
          );
          execution.status = 'completed';
          execution.completedAt = new Date();
          await this.executionRepository.save(execution);
      ...
      // ==================== MULTI-CONDITION NODE ====================
        ...
        for (let i = 0; i < conditions.length; i++) {
          ...
          switch (cond.operator) {
            case 'equals':
              met = String(actualValue) === String(cond.value);
              break;
            case 'contains':
              met = String(actualValue).includes(String(cond.value));
              break;
            case 'not_equals':
              met = String(actualValue) !== String(cond.value);
              break;
            case 'starts_with':
              met = String(actualValue).startsWith(String(cond.value));
              break;
            case 'gt':
              met = Number(actualValue) > Number(cond.value);
              break;
            case 'lt':
              met = Number(actualValue) < Number(cond.value);
              break;
        ...
        const handleId =
          matchedIndex >= 0 ? `condition-${matchedIndex}` : 'else';
        console.log(
          `[Flow] MultiCondition node ${nextNode.id}: matched index=${matchedIndex}, handle=${handleId}`,
        );
      ...
      // ==================== WEBHOOK NODE ====================
        ...
        console.log(
          `[Flow] Webhook node ${nextNode.id}: ${method} ${webhookUrl}`,
        );

        if (webhookUrl) {
          try {
            const { instance, contact } =
              await this.getExecutionContext(execution);
            ...
            console.error(
              `[Flow] Webhook error at node ${nextNode.id}:`,
              webhookErr.message,
            );
            execution.logs.push({
              nodeId: nextNode.id,
      ...
      // ==================== TEMPLATE NODES (templateText, templateButton, template) ====================
        ...
        ['templateText', 'templateButton', 'template'].includes(nodeType)
      ) {
        const { instance, contact } = await this.getExecutionContext(execution);
        const nodeData = nextNode.data;
        const templateName =
          nodeData?.config?.templateName || nodeData?.config?.name || '';
        ...
        const headerUrl =
          nodeData?.config?.headerUrl || nodeData?.config?.mediaUrl;

        console.log(
          `[Flow] Template node ${nextNode.id}: template=${templateName}`,
        );
        ...
        if (instance?.status === 'connected' && contact && templateName) {
          try {
            ...
            if (
              ...
              const body =
                nodeData?.config?.body || `[Template: ${templateName}]`;
              const res = await provider.sendText(
                instance.instanceName,
                contact.phone,
                body,
            ...
            console.error(
              `[Flow] Template send error at node ${nextNode.id}:`,
              tplErr.message,
            );
            execution.logs.push({
              nodeId: nextNode.id,
          ...
          execution.logs.push({
            ...
            timestamp: new Date().toISOString(),
            data: {
              reason: !templateName ? 'no_template' : 'instance_not_connected',
            },
          });
        }
      }
      ...
      // ==================== WHATSAPP FLOW NODE ====================
        ...
        if (instance?.status === 'connected' && contact) {
          ...
          if (provider.sendFlow) {
            const nodeData = nextNode.data;
            const flowId = nodeData?.config?.flowId || '';
            const flowButtonText =
              nodeData?.config?.flowButtonText || 'Preencher';
            const messageText =
              nodeData?.config?.messageText || 'Preencha o formulário';
            const flowScreen = nodeData?.config?.flowScreen || 'UNION';
            const flowToken =
              nodeData?.config?.flowToken ||
              `flow-${execution.id}-${Date.now()}`;
            ...
            const res = await provider.sendFlow(
              instance.instanceName,
              contact.phone,
              {
                flowId,
            ...
            if (res?.messageId) {
              ...
              await this.inboxService
                ...
                .catch((err) =>
                  console.error(
                    `[Flow] Error saving flow message to inbox:`,
                    err.message,
                  ),
                );
            }
            ...
            execution.variables = {
              ...
              waitingForAnswer: true,
              waitingNodeId: nextNode.id,
              waitingSaveTo: 'lastFlowResponse',
              flowToken,
              flowId,
            };
            ...
            execution.logs.push({
              ...
              action: 'flow_sent',
              timestamp: new Date().toISOString(),
              data: { flowId, flowToken, messageId: res?.messageId },
            });

            await this.executionRepository.save(execution);
            return; // Wait for user submission webhook
          } else {
            console.warn(
              `[Flow] Provider ${instance.provider} does not support WhatsApp Flows. Sending fallback text.`,
            );
            const nodeData = nextNode.data;
            const messageText =
              nodeData?.config?.messageText || 'Preencha o formulário';
            const res = await provider.sendText(
              instance.instanceName,
              contact.phone,
              `${messageText} (Acesse pelo app do WhatsApp oficial para preencher)`,
            );
            if (res?.messageId) {
              await this.updateCampaignContactMessageId(
                execution,
            ...
            execution.logs.push({
              nodeId: nextNode.id,
              action: 'flow_sent_fallback',
              timestamp: new Date().toISOString(),
            });
          }
        } else {
          console.warn(
            `[Flow] Instance not connected for WhatsApp Flow node ${nextNode.id}`,
          );
        }
      }
      ...
      // ==================== SAVE INFO NODE ====================
        ...
        const field =
          nodeData?.config?.field || nodeData?.config?.fieldName || '';
        const value =
          nodeData?.config?.value || nodeData?.config?.fieldValue || '';

        if (field) {
          execution.variables = { ...execution.variables, [field]: value };
          console.log(
            `[Flow] SaveInfo node ${nextNode.id}: saved ${field}=${value}`,
          );
        }
      ...
      // ==================== LIMIT EXECUTION NODE ====================
        ...
        if (period === 'day') {
          ...
          countQuery = countQuery.andWhere('exec.startedAt >= :dayStart', {
            dayStart,
          });
        } else if (period === 'hour') {
          const hourStart = new Date(Date.now() - 3600000);
          countQuery = countQuery.andWhere('exec.startedAt >= :hourStart', {
            hourStart,
          });
        }
        ...
        console.log(
          `[Flow] LimitExecution node ${nextNode.id}: max=${maxExecutions}, period=${period}, currentCount=${executionCount}, passed=${passed}`,
        );
        ...
        if (!passed) {
          ...
          await this.markCampaignContactAsSent(execution);
          console.log(
            `[Flow] ⛔ Execution stopped by limitExecution: ${executionCount}/${maxExecutions} (${period})`,
          );
          return;
        }
      }
      ...
      // ==================== MOVE FLOW NODE ====================
        ...
        const nodeData = nextNode.data;
        const targetFlowId =
          nodeData?.config?.flowId ||
          nodeData?.config?.targetFlowId ||
          nodeData?.config?.targetFlow ||
          '';

        console.log(
          `[Flow] MoveFlow node ${nextNode.id}: target flow=${targetFlowId}`,
        );
        ...
        if (targetFlowId) {
          ...
          try {
            const tenantId = (
              await this.flowRepository.findOne({
                where: { id: execution.flowId },
              })
            )?.tenantId;
            if (tenantId) {
      ...
      // ==================== AI NODES (ChatGPT, Gemini, etc) ====================
        ...
        const prompt =
          ...
          nodeData?.config?.systemPrompt ||
          'Você é um assistente virtual útil.';
        const userMessage = execution.variables?.lastAnswer || 'Olá';

        console.log(
          `[Flow] AI node ${nextNode.id}: type=${nodeType}, promptLength=${prompt.length}`,
        );
        ...
        if (token && contact) {
          try {
            ...
            if (
              nodeData?.config?.autoSend !== false &&
              instance?.status === 'connected'
            ) {
              const whatsappProvider = this.whatsappFactory.getProvider(
                instance.provider,
              );
            ...
            console.error(
              `[Flow] AI Error at node ${nextNode.id}:`,
              aiErr.message,
            );
            execution.logs.push({
              nodeId: nextNode.id,
      ...
      // ==================== RANDOMIZER NODE ====================
        ...
        if (outEdges.length > 0) {
          const nodeData = nextNode.data;
          const configPaths: Array<{ name: string; weight: number }> =
            nodeData?.config?.paths || [];
          ...
          const hasWeights =
            configPaths.length > 0 && configPaths.some((p) => p.weight > 0);
          ...
          if (hasWeights) {
            ...
            const edgesWithWeights = outEdges.map((edge) => {
              ...
              if (edge.sourceHandle && edge.sourceHandle.startsWith('path-')) {
                pathIndex = parseInt(
                  edge.sourceHandle.replace('path-', ''),
                  10,
                );
              }
              const weight =
                pathIndex >= 0 && pathIndex < configPaths.length
                  ? Math.max(0, configPaths[pathIndex].weight || 0)
                  : 0;
              return { edge, pathIndex, weight };
            });

            const totalWeight = edgesWithWeights.reduce(
              (sum, ew) => sum + ew.weight,
              0,
            );
            ...
            if (totalWeight > 0) {
              ...
              weightsUsed = edgesWithWeights
                .map((ew) => `path-${ew.pathIndex}:${ew.weight}%`)
                .join(', ');
            } else {
              // All weights are 0 — fall back to uniform
              const uniformIndex = Math.floor(Math.random() * outEdges.length);
              selectedEdge = outEdges[uniformIndex];
              let pathIndex = -1;
              if (
                selectedEdge.sourceHandle &&
                selectedEdge.sourceHandle.startsWith('path-')
              ) {
                pathIndex = parseInt(
                  selectedEdge.sourceHandle.replace('path-', ''),
                  10,
                );
              }
              selectedIndex = pathIndex >= 0 ? pathIndex : uniformIndex;
          ...
          const pathName =
            selectedIndex >= 0 && selectedIndex < configPaths.length
              ? configPaths[selectedIndex].name
              : `path-${selectedIndex}`;

          console.log(
            `[Flow] 🎲 Randomizer node ${nextNode.id}: [${selectionMethod}] selected "${pathName}" (index ${selectedIndex}) -> ${selectedEdge.target} | weights: [${weightsUsed}]`,
          );
          ...
          const rNextNode = flow.nodes.find(
      ...
      // ==================== FAKE CALL NODE ====================
        ...
        console.log(
          `[Flow] FakeCall node ${nextNode.id}: simulating call to ${contact?.phone}`,
        );
        execution.logs.push({
          nodeId: nextNode.id,
      ...
      // ==================== CONTACTS NODE ====================
        ...
        const tagName =
          nodeData?.config?.tag || nodeData?.config?.tagName || '';
        const tagIds = nodeData?.config?.tagIds || [];

        console.log(
          `[Flow] Contacts node ${nextNode.id}: action=${action}, tag=${tagName}, tagIds=${JSON.stringify(tagIds)}`,
        );
        ...
        try {
          if (execution.contactId) {
            ...
            if (tenantId) {
              if (action === 'addTag' && tagIds.length > 0) {
                await this.contactsService.addTagsToContact(
                  execution.contactId,
                  tagIds,
                );
                console.log(
                  `[Flow] ✅ Added ${tagIds.length} tags to contact ${execution.contactId}`,
                );
              } else if (action === 'addTag' && tagName) {
                // Find or create tag by name, then add
                const existingTags =
                  await this.contactsService.findAllTags(tenantId);
                let tag = existingTags.find((t: any) => t.name === tagName);
                if (!tag) {
                  tag = await this.contactsService.createTag(tenantId, {
                    name: tagName,
                  });
                }
                await this.contactsService.addTagsToContact(
                ...
                console.log(
                  `[Flow] ✅ Added tag "${tagName}" to contact ${execution.contactId}`,
                );
                ...
                console.log(
                  `[Flow] ✅ Removed ${tagIds.length} tags from contact ${execution.contactId}`,
                );
              } else if (action === 'block') {
                await this.contactsService.blockContact(
                  tenantId,
                  execution.contactId,
                );
                console.log(`[Flow] 🚫 Contact ${execution.contactId} blocked`);
              } else if (action === 'unblock') {
                await this.contactsService.unblockContact(
                  tenantId,
                  execution.contactId,
                );
                console.log(
                  `[Flow] 🔓 Contact ${execution.contactId} unblocked`,
                );
              } else if (action === 'optOut') {
                await this.contactsService.setOptOut(
                  tenantId,
                  execution.contactId,
                  true,
                );
                console.log(
                  `[Flow] 🚪 Contact ${execution.contactId} marked as opt-out`,
                );
              } else if (action === 'changeCategory') {
                const newCategory =
                  nodeData?.config?.categoryName ||
                  nodeData?.config?.category ||
                  '';
                if (newCategory) {
                  ...
                  console.log(
                    `[Flow] 🏷️ Contact ${execution.contactId} category changed to ${newCategory}`,
                  );
                }
              }
            }
          ...
          console.error(
            `[Flow] Tag operation error at node ${nextNode.id}:`,
            tagErr.message,
          );
        }
      ...
      // ==================== START NODE (skip) ====================
        ...
        console.log(
          `[Flow] Start node ${nextNode.id}: skipping (already started)`,
        );
        execution.logs.push({
          nodeId: nextNode.id,
          action: 'start_passed',
      ...
      // ==================== UNKNOWN NODE TYPE ====================
        ...
        console.warn(
          `[Flow] Unknown node type at ${nextNode.id}: "${nodeType}" — passing through`,
        );
        execution.logs.push({
          nodeId: nextNode.id,
      ...
      if (nodeType === 'end') {
        ...
        await this.markCampaignContactAsSent(execution);
        console.log(
          `[Flow] [${executionId.slice(0, 8)}] ✅ Flow completed at end node ${nextNode.id}`,
        );
        return;
      }
      ...
      console.log(
        `[Flow] [${executionId.slice(0, 8)}] → Advancing from ${nextNode.id} to next...`,
      );
      await this.processExecution(executionId);
    } catch (error) {
      console.error(
        `[Flow] Error processing node ${nextNode.id} (type: ${nodeType}):`,
        error,
      );
      ...
      if (execution.variables?.campaignContactId) {
        try {
          ...
          console.log(
            `[Flow] Campaign contact ${execution.variables.campaignContactId} marked as failed.`,
          );
          ...
          console.error(
            `[Flow] Failed to update campaign contact status:`,
            updateErr.message,
          );
        }
      }
  ...
  async getExecutions(tenantId: string, flowId?: string) {
    ...
    return query.orderBy('execution.started_at', 'DESC').limit(100).getMany();
  }
  ...
  async testFlow(
    ...
    if (!contact) {
      throw new BadRequestException(
        'Erro ao criar ou encontrar contato de teste.',
      );
    }
    ...
    for (const flow of activeFlows) {
      ...
      if (hasMatch) {
        ...
        if (existingExecution) {
          console.log(
            `[Flow] Trigger skipped for flow "${flow.name}" — contact already has active execution ${existingExecution.id.slice(0, 8)} (status: ${existingExecution.status})`,
          );
          continue;
        }

        console.log(
          `[Flow] Trigger matched for flow "${flow.name}" with message: "${message}"`,
        );
        // Start execution
