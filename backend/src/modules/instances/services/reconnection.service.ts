...
export class ReconnectionService implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit() {
    this.logger.log(
      '🔄 ReconnectionService iniciado — monitoramento ativo de desconexões',
    );
    ...
    setTimeout(() => {
      this.monitorInterval = setInterval(
        () =>
          this.runReconnectionCycle().catch((e) =>
            this.logger.error(`Erro no ciclo de reconexão: ${e.message}`),
          ),
      ...
      this.runReconnectionCycle().catch((e) =>
        this.logger.error(`Erro na reconexão inicial: ${e.message}`),
      );
    }, 30000);
    ...
    setTimeout(() => {
      this.syncInterval = setInterval(
        () =>
          this.runStateSyncCycle().catch((e) =>
            this.logger.error(`Erro no ciclo de sync: ${e.message}`),
          ),
      ...
      this.runStateSyncCycle().catch((e) =>
        this.logger.error(`Erro no sync inicial: ${e.message}`),
      );
    }, 60000);
  }
  ...
  async runReconnectionCycle(): Promise<void> {
    ...
    this.logger.log(
      `🔍 Verificando ${disconnectedInstances.length} chips desconectados/reconectando...`,
    );
    ...
    for (const instance of disconnectedInstances) {
      try {
        ...
        this.logger.error(
          `Erro ao processar ${instance.instanceName}: ${e.message}`,
        );
      }
    }
  }

  private async processDisconnectedInstance(
    instance: Instance,
    now: Date,
  ): Promise<void> {
    // Se está bloqueado por backoff, verificar se o tempo passou
    if (
      instance.reconnectLockedUntil &&
      new Date(instance.reconnectLockedUntil) > now
    ) {
      const remainingMs =
        new Date(instance.reconnectLockedUntil).getTime() - now.getTime();
      this.logger.debug(
        `⏳ ${instance.instanceName} aguardando backoff (${Math.round(remainingMs / 60000)}min restantes)`,
      );
      return;
    }
    ...
    if (
      instance.status === InstanceStatus.RECONNECTING &&
      instance.lastReconnectAttemptAt
    ) {
      const timeSinceAttempt =
        now.getTime() - new Date(instance.lastReconnectAttemptAt).getTime();
      if (timeSinceAttempt < this.RECONNECTING_TIMEOUT_MS) {
        this.logger.debug(
          `⏳ ${instance.instanceName} ainda em reconexão automática do Baileys (${Math.round(timeSinceAttempt / 60000)}min)`,
        );
        return;
      }
    }
  ...
  private async attemptReconnect(instance: Instance, now: Date): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        this.logger.warn(
          `Não consegui verificar status de ${instance.instanceName} no provider: ${e.message}`,
        );
      }
      ...
      if (providerStatus?.status === InstanceStatus.CONNECTED) {
        this.logger.log(
          `✅ ${instance.instanceName} já está conectado no provider! Atualizando banco...`,
        );
        await this.instanceRepo.update(instance.id, {
          ...
          phone:
            providerStatus.phoneNumber?.replace('@s.whatsapp.net', '') ||
            instance.phone,
        });
      ...
      if (providerStatus && providerStatus.exists === false) {
        ...
        this.logger.warn(
          `${instance.instanceName} não existe no container. Recriando...`,
        );
        try {
          const config =
            instance.channelType === 'official' ? instance.metaConfig : {};
          await provider.createInstance(instance.instanceName, config);
        } catch (createErr) {
          this.logger.error(
            `Falha ao recriar ${instance.instanceName}: ${createErr.message}`,
          );
        }
      }
      ...
      this.logger.log(
        `📡 ${instance.instanceName} aguardando reconexão (backoff: ${backoffMinutes}min)`,
      );
    } catch (err) {
      this.logger.error(
        `❌ Falha na reconexão de ${instance.instanceName}: ${err.message}`,
      );
      await this.instanceRepo.update(instance.id, {
        reconnectAttempts: attempts,
        lastReconnectAttemptAt: now,
  ...
  async runStateSyncCycle(): Promise<void> {
    this.logger.debug(
      '🔄 Executando ciclo de sincronização de estado BD ↔ Provider...',
    );
    ...
    for (const instance of connectedInDB) {
      try {
        ...
        if (providerStatus.status !== InstanceStatus.CONNECTED) {
          ...
          await this.instanceRepo.update(instance.id, {
            status:
              providerStatus.status === InstanceStatus.DISCONNECTED
          ...
          const phone = providerStatus.phoneNumber.replace(
            '@s.whatsapp.net',
            '',
          );
          await this.instanceRepo.update(instance.id, { phone });
          this.logger.log(
            `📱 Sincronizando telefone de ${instance.instanceName}: ${phone}`,
          );
        }
      } catch (e) {
        this.logger.warn(
          `Erro ao sincronizar ${instance.instanceName}: ${e.message}`,
        );
      }
    }
    ...
    if (divergences > 0) {
      this.logger.warn(
        `⚠️ Ciclo de sync encontrou ${divergences} divergências e as corrigiu`,
      );
    } else {
      this.logger.debug(
        `✅ Sync concluído — ${connectedInDB.length} chips confirmados como conectados`,
      );
    }
  }
  ...
  async forceReconnect(
    ...
    return {
      success: true,
      message: `Reconexão forçada para ${instance.instanceName}`,
    };
  }
  ...
  async reconnectAll(): Promise<{
    ...
    this.logger.log(
      `🔄 Iniciando reconexão forçada de ${instances.length} chips...`,
    );
