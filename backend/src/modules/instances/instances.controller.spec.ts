...
describe('InstancesController', () => {
  ...
  beforeEach(async () => {
    instancesService = {
      ...
      getQrCode: jest
        .fn()
        .mockResolvedValue('data:image/png;base64,mockQrCode'),
      ...
      getAvailableProviders: jest
        .fn()
        .mockReturnValue(['waha', 'evolution', 'wwebjs']),
      findAllProxies: jest.fn().mockResolvedValue([]),
      createProxy: jest
        .fn()
        .mockResolvedValue({ id: 'proxy-123', host: 'test.com', port: 1080 }),
    };

    const module: TestingModule = await Test.createTestingModule({
      controllers: [InstancesController],
      providers: [{ provide: InstancesService, useValue: instancesService }],
    }).compile();
  ...
  describe('findOne', () => {
    it('should return instance by id', async () => {
      ...
      expect(instancesService.findOne).toHaveBeenCalledWith(
        'instance-123',
        'tenant-123',
      );
    });
  ...
  describe('getStatus', () => {
    it('should return instance status', async () => {
      ...
      expect(instancesService.getStatus).toHaveBeenCalledWith(
        'instance-123',
        'tenant-123',
      );
    });
  ...
  describe('getQrCode', () => {
    it('should return QR code', async () => {
      ...
      expect(instancesService.getQrCode).toHaveBeenCalledWith(
        'instance-123',
        'tenant-123',
      );
    });
  ...
  describe('delete', () => {
    it('should delete instance', async () => {
      ...
      expect(instancesService.delete).toHaveBeenCalledWith(
        'instance-123',
        'tenant-123',
      );
    });
  ...
  describe('Proxies', () => {
    describe('findAllProxies', () => {
      it('should return all proxies', async () => {
        ...
        expect(instancesService.findAllProxies).toHaveBeenCalledWith(
          'tenant-123',
        );
      });
    });
    ...
    describe('createProxy', () => {
      it('should create a new proxy', async () => {
        ...
        expect(instancesService.createProxy).toHaveBeenCalledWith(
          'tenant-123',
          data,
        );
      });
