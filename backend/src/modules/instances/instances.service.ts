...
export class InstancesService {
  ...
  async findAll(tenantId: string) {
    try {
      ...
      this.logger.error(
        `Failed to find instances: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async create(
    ...
    if (existing) {
      throw new ConflictException(
        `Já existe uma instância com o nome "${data.instanceName}". Por favor, escolha outro nome ou exclua a existente.`,
      );
    }
    ...
    this.logger.log(
      `Creating instance via ${providerType}: ${data.instanceName}`,
    );
    ...
    try {
      ...
      if (!isOfficial && !allocatedProxyId) {
        this.logger.log(
          `[PROXY AUTO-ALLOC] Buscando proxy livre no pool para o tenant ${tenantId}`,
        );

        // 1. Tentar encontrar um proxy já existente e não associado a nenhuma instância
        const unassignedProxy =
          await this.proxiesService.getUnassignedProxy(tenantId);

        if (unassignedProxy) {
          this.logger.log(
            `[PROXY AUTO-ALLOC] Reutilizando proxy livre existente: ${unassignedProxy.host}:${unassignedProxy.port}`,
          );
          allocatedProxyId = unassignedProxy.id;
        } else {
          this.logger.log(
            `[PROXY AUTO-ALLOC] Nenhum proxy livre encontrado. Provisionando um novo proxy ISP da IPRoyal...`,
          );
          // 2. Chamar o serviço para comprar/criar um novo proxy
          const newProxy =
            await this.proxiesService.buyProxyFromProvider(tenantId);
      ...
      if (allocatedProxyId) {
        await this.proxiesService.assignProxy(
          tenantId,
          allocatedProxyId,
          instance.id,
        );
      ...
      if (error.code === '23505' || error.message?.includes('duplicate')) {
        throw new ConflictException(
          `Já existe uma instância com o nome "${data.instanceName}". Por favor, escolha outro nome.`,
        );
      }
  ...
  async getQrCode(id: string, tenantId: string) {
    ...
    try {
      ...
      if (
        ...
        this.logger.warn(
          `Instance '${instance.instanceName}' missing or errored in provider container. Re-creating...`,
        );
        const config =
        ...
        try {
          ...
          this.logger.error(
            `Auto-healing creation failed for ${instance.instanceName}: ${createErr.message}`,
          );
        }
  ...
  async getPairingCode(
    ...
    if (provider.providerType !== 'evolution') {
      throw new ConflictException(
        'Pairing code connection is only supported by Evolution API provider',
      );
    }
    ...
    if (typeof adapter.getPairingCode !== 'function') {
      throw new ConflictException(
        'Pairing code connection is not supported by the active provider version',
      );
    }
    ...
    this.logger.log(
      `Forcing clean reset of instance '${instance.instanceName}' in container to prepare for pairing code...`,
    );
    try {
      ...
      this.logger.warn(
        `Failed to delete instance '${instance.instanceName}' before pairing (normal if not present): ${deleteErr.message}`,
      );
    }
    ...
    try {
      ...
      this.logger.error(
        `Recreation failed for '${instance.instanceName}' during pairing setup: ${createErr.message}`,
      );
    }

    try {
      const result = await adapter.getPairingCode(
  ...
  async getStatus(id: string, tenantId: string) {
    ...
    if (
      status.status === InstanceStatus.CONNECTED &&
    ...
    if (shouldSave) {
      ...
      if (
        status.status === InstanceStatus.CONNECTED &&
        (!instance.warmupDay || instance.warmupDay === 0)
      ) {
        this.logger.log(
          `Auto-scanning maturity for veteran chip ${instance.instanceName}`,
        );
        // Não bloqueamos a requisição HTTP para isso, rodamos em background
        this.scanMaturity(id, tenantId).catch((e) =>
          this.logger.warn(`Auto-scan failed: ${e.message}`),
        );
      }
    }
  ...
  async scanMaturity(id: string, tenantId: string) {
    ...
    this.logger.log(
      `[SCAN] Starting maturity scan for ${instance.instanceName}...`,
    );
    ...
    try {
      ...
      if (calculatedDay > currentDay) {
        this.logger.log(
          `[SCAN] Chip ${instance.instanceName} promoted from Day ${currentDay} to Day ${calculatedDay} based on ${chatCount} chats!`,
        );
        ...
        if (
          calculatedDay >= 14 &&
          (!instance.dailyLimit || instance.dailyLimit < 150)
        ) {
  ...
  async update(
    id: string,
    tenantId: string,
    data: Partial<Instance> & { chipDetails?: any },
  ) {
    const instance = await this.findOne(id, tenantId);
    ...
    if (data.metaConfig) {
      ...
      if ('customDailyLimit' in data.metaConfig) {
        ...
        if (!isNaN(customLimit) && customLimit > 0) {
          ...
          data.dailyLimit = Math.max(
            5,
            Math.round(customLimit * (1 + variation)),
          );
