...
export class PayloadNormalizerService {
  ...
  private normalizeGeneric(
    ...
    return {
      ...
      phoneE164: this.formatPhoneE164(
        customer.phone || shipping.phone || order.phone || '',
      ),
      trackingCode:
      ...
      occurredAt: new Date(
        payload.occurred_at || payload.created_at || new Date(),
      ),
    };
  }
  ...
  private normalizeWooCommerce(
    ...
    return {
      ...
      trackingCode: order.meta_data?.find(
        (m: any) => m.key === '_tracking_number',
      )?.value,
      trackingUrl: order.meta_data?.find((m: any) => m.key === '_tracking_url')
        ?.value,
      itemsCount: order.line_items?.length,
      ...
      providerId: order.id?.toString(),
      occurredAt: new Date(
        order.date_modified || order.date_created || new Date(),
      ),
    };
  }

  private normalizeYampi(
    payload: Record<string, any>,
    eventType?: string,
  ): NormalizedPayload {
