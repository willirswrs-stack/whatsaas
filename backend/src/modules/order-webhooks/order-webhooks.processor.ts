...
export class OrderWebhooksProcessor extends WorkerHost {
  ...
  async process(
    job: Job<ProcessEventJobData | SendMessageJobData>,
  ): Promise<any> {
    const { name, data } = job;
  ...
  private async sendMessage(data: SendMessageJobData): Promise<void> {
    ...
    try {
      ...
      this.logger.log(
        `✅ Message sent: ${message.id} to ${message.toPhoneE164}`,
      );
    } catch (error) {
      const duration = Date.now() - startTime;
      ...
      if (message.tries >= MAX_RETRIES) {
        ...
        await this.outboxRepo.save(message);
        this.logger.error(
          `❌ Message permanently failed: ${message.id} - ${error.message}`,
        );
      } else {
        message.status = MessageStatus.RETRYING;
        message.nextRetryAt = new Date(
          Date.now() + RETRY_DELAYS[message.tries - 1],
        );
        await this.outboxRepo.save(message);
