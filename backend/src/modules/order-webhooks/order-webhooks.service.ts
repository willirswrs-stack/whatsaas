...
import {
  WebhookEventType,
  DEFAULT_EVENT_TYPES,
} from './entities/webhook-event-type.entity';
import {
  WebhookEventMapping,
  SendMode,
} from './entities/webhook-event-mapping.entity';
import {
  WebhookEventInbox,
  ProcessedStatus,
} from './entities/webhook-event-inbox.entity';
import {
  MessageOutbox,
  MessageStatus,
  MessageProvider,
} from './entities/message-outbox.entity';
...
export class OrderWebhooksService {
  ...
  async createMapping(
    ...
    if (!eventType) {
      throw new BadRequestException(
        `Event type '${dto.eventTypeCode}' not found`,
      );
    }
  ...
  async findInboxEvents(
    ...
    if (filters.status) {
      qb.andWhere('inbox.processedStatus = :status', {
        status: filters.status,
      });
    }
    if (filters.startDate) {
      qb.andWhere('inbox.receivedAt >= :startDate', {
  ...
  async findOutboxMessages(
    ...
    if (filters.phone) {
      qb.andWhere('outbox.toPhoneE164 LIKE :phone', {
        phone: `%${filters.phone}%`,
      });
    }
    if (filters.orderId) {
      qb.andWhere('outbox.orderId LIKE :orderId', {
        orderId: `%${filters.orderId}%`,
      });
    }
    if (filters.startDate) {
      qb.andWhere('outbox.createdAt >= :startDate', {
        startDate: filters.startDate,
      });
    }
    if (filters.endDate) {
  ...
  async handleInboundWebhook(
    ...
    this.logger.log(
      `Event received: ${eventTypeCode} for tenant ${integration.tenantId}`,
    );
    ...
    try {
      ...
      for (const mapping of mappings) {
        ...
        if (
          ...
          event.processingLog.push({
            ...
            details: {
              ...
              reason: 'Match rules not satisfied',
            },
          });
          continue;
        }
        ...
        if (mapping.forwardToN8n && mapping.n8nWebhookUrl) {
          this.forwardToN8n(mapping.n8nWebhookUrl, event, normalizedData).catch(
            (err) => {
  ...
  private validateSignature(
    ...
    switch (integration.signatureType) {
      ...
      case SignatureType.TOKEN_HEADER:
        const headerValue =
          headers[integration.signatureHeader?.toLowerCase() || ''];
        return headerValue === integration.inboundSecret;

      case SignatureType.HMAC_SHA256:
        const signature =
          headers[integration.signatureHeader?.toLowerCase() || 'x-signature'];
        ...
        return (
          expectedSig === cleanSig || `sha256=${expectedSig}` === signature
        );
  ...
  private evaluateMatchRules(
    ...
    for (const [key, expectedValue] of Object.entries(rules)) {
      const actualValue =
        this.normalizer.getNestedValue(payload, key) ||
      ...
      if (typeof expectedValue === 'string') {
        ...
        if (expectedValue.exists !== undefined) {
          if ((actualValue !== undefined) !== expectedValue.exists)
            return false;
        }
        if (expectedValue.includes !== undefined) {
          if (!String(actualValue || '').includes(expectedValue.includes))
            return false;
        }
      }
    }
  ...
  async getStatistics(tenantId: string): Promise<{
    ...
    const [
      ...
      this.outboxRepo.count({
        where: { tenantId, status: MessageStatus.QUEUED },
      }),
      this.outboxRepo.count({
        where: { tenantId, status: MessageStatus.SENT },
      }),
      this.outboxRepo.count({
        where: { tenantId, status: MessageStatus.FAILED },
      }),
    ]);
  ...
  async testWebhook(
    ...
    const detectedEventType = this.normalizer.detectEventType(
      integration.provider,
      payload,
    );
