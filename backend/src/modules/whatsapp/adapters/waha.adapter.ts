...
export class WahaAdapter implements IWhatsAppProvider {
  ...
  async getStatus(instanceName: string): Promise<InstanceStatus> {
    try {
      const response = await this.request<WahaSessionResponse>(
        'GET',
    ...
    const response = await this.request<WahaSendTextResponse>(
      ...
      `/api/default/sendFile`,
      {
        ...
        file: {
          ...
          filename:
            media.filename ||
            `file.${media.type === 'video' ? 'mp4' : media.type === 'audio' ? 'mp3' : media.type === 'image' ? 'jpg' : 'pdf'}`,
        },
  ...
  async sendPresence(
    ...
    try {
      const endpoint =
        presence === 'paused'
          ? `/api/default/stopTyping`
          : `/api/default/startTyping`;
      await this.request('POST', endpoint, { chatId: this.formatJid(to) });
      if (presence !== 'paused') {
        await this.sleep(durationMs);
        await this.request('POST', `/api/default/stopTyping`, {
          chatId: this.formatJid(to),
        });
      }
    } catch (error: any) {
  ...
  async isOnWhatsApp(instanceName: string, phone: string): Promise<boolean> {
    try {
      const response = await this.request<WahaCheckNumberResponse[]>(
        'POST',
        `/api/default/contacts/check`,
        {
  ...
  async joinGroup(instanceName: string, inviteUrl: string): Promise<any> {
    try {
      this.logger.log(
        `[WAHA] Joining group via invite: ${inviteUrl} for instance ${instanceName}`,
      );
      const sessionName = 'default';
      return await this.request(
        'POST',
        `/api/${sessionName}/groups/accept-invite`,
        {
