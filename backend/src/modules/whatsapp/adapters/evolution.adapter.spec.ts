...
describe('EvolutionAdapter', () => {
  ...
  describe('createInstance', () => {
    ...
    it('should reuse existing instance if found', async () => {
      ...
      mockFetch.mockResolvedValueOnce(
        mockResponse([{ name: 'test-instance', id: 'existing-123' }]),
      );
  ...
  describe('deleteInstance', () => {
    ...
    it('should not throw if instance already deleted (404)', async () => {
      ...
      await expect(
        adapter.deleteInstance('test-instance'),
  ...
  describe('sendText', () => {
    it('should send text message successfully', async () => {
      ...
      mockFetch
        .mockResolvedValueOnce(
          mockResponse([{ exists: true, jid: '5511999999999@s.whatsapp.net' }]),
        ) // resolveAndValidateNumber
        .mockResolvedValueOnce(
          mockResponse({
            key: { id: 'msg-123' },
            status: 'PENDING',
    ...
    it('should bypass resolveAndValidateNumber if the recipient JID contains @lid', async () => {
      // Arrange
      mockFetch.mockResolvedValueOnce(
        mockResponse({
          key: { id: 'msg-lid-123' },
      ...
      // Assert
      expect(result.messageId).toBe('msg-lid-123');
      expect(result.status).toBe('sent');
      expect(mockFetch).toHaveBeenCalledWith(
        'http://evolution-api:8080/message/sendText/test-instance',
        expect.objectContaining({
  ...
  describe('sendPresence', () => {
    it('should send composing presence', async () => {
      ...
      expect(mockFetch).toHaveBeenLastCalledWith(
        'http://evolution-api:8080/chat/sendPresence/test-instance',
  ...
  describe('isOnWhatsApp', () => {
    it('should return true if number is on WhatsApp', async () => {
      ...
      // Act
      const result = await adapter.isOnWhatsApp(
        'test-instance',
        '5511999999999',
      );
    ...
    it('should return false if number is not on WhatsApp', async () => {
      // Arrange
      mockFetch.mockResolvedValueOnce(mockResponse([{ exists: false }]));
  ...
  describe('formatPhone', () => {
    it('should format phone number by removing non-digits', async () => {
      ...
      mockFetch
        ...
        .mockResolvedValueOnce(mockResponse({ key: { id: 'msg-123' } }));
  ...
  describe('sendFlow', () => {
    it('should send WhatsApp Flow message via sendInteractive endpoint', async () => {
      // Arrange
      mockFetch
        .mockResolvedValueOnce(
          mockResponse([
        ...
        ) // isOnWhatsApp/resolveAndValidateNumber
        .mockResolvedValueOnce(mockResponse({ key: { id: 'msg-flow-123' } })); // sendInteractive

      // Act
      const result = await adapter.sendFlow('test-instance', '5511999999999', {
        flowId: '12345678',
        flowButtonText: 'Clique Aqui',
        messageText: 'Por favor preencha',
        flowToken: 'token-test',
        flowScreen: 'SCREEN_1',
      });

      // Assert
      expect(result.messageId).toBe('msg-flow-123');
      expect(result.status).toBe('sent');
      expect(mockFetch).toHaveBeenLastCalledWith(
        'http://evolution-api:8080/message/sendInteractive/test-instance',
        expect.objectContaining({
          method: 'POST',
          body: JSON.stringify({
            number: '5511999999999',
  ...
  describe('Request headers', () => {
    it('should include proper headers in requests', async () => {
      ...
      mockFetch
        .mockResolvedValueOnce(mockResponse([])) // fetchInstances
        .mockResolvedValueOnce(
          mockResponse({ instance: { instanceName: 'test' } }),
        ); // create
