...
export class EvolutionAdapter implements IWhatsAppProvider {
  ...
  async createInstance(
    ...
    try {
      ...
      const existingInstances = await this.request(
        'GET',
      ...
      if (
        ...
        this.logger.log(
          `Instance '${instanceName}' already exists (caught error)`,
        );
        return {
      ...
      this.logger.error(
        `Failed to create Evolution instance: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async getQrCode(instanceName: string): Promise<string> {
    ...
    for (let i = 0; i < maxRetries; i++) {
      try {
        ...
        this.logger.debug(
          `QR response (attempt ${i + 1}): ${JSON.stringify(response).substring(0, 100)}...`,
        );
        ...
        this.logger.error(
          `QR Fetch Error (${i + 1}/${maxRetries}): ${error.message}`,
        );
  ...
  async getPairingCode(
    ...
    this.logger.log(
      `Generating pairing code for ${instanceName} using phone primary: ${formatted}${alternate ? `, alternate: ${alternate}` : ''}`,
    );
    ...
    for (let i = 0; i < maxRetries; i++) {
      for (const phone of phoneNumbersToTry) {
        try {
          this.logger.log(
            `Attempting to generate pairing code for ${instanceName} using phone ${phone} (attempt ${i + 1}/${maxRetries})`,
          );
          const response = await this.request(
            'GET',
            `/instance/connect/${instanceName}?number=${phone}`,
          );

          this.logger.debug(
            `Pairing Code response for ${phone} (attempt ${i + 1}): ${JSON.stringify(response).substring(0, 100)}...`,
          );

          const code = extractPairingCode(response);
          if (code) {
            this.logger.log(
              `Successfully generated pairing code for ${instanceName} using phone ${phone} on attempt ${i + 1}: ${code}`,
            );
            return { pairingCode: code, phone };
          }

          this.logger.log(
            `Pairing code not ready yet for ${instanceName} with phone ${phone} on attempt ${i + 1}.`,
          );
        } catch (error: any) {
          this.logger.warn(
            `Pairing Code Fetch Error for phone ${phone} (${i + 1}/${maxRetries}): ${error.message}`,
          );
        }
      }
    ...
    throw new Error(
      'Código de pareamento não gerado a tempo. Certifique-se de que o número de telefone está ativo no WhatsApp e tente novamente.',
    );
  }
  ...
  async getStatus(instanceName: string): Promise<InstanceStatus> {
    try {
      ...
      let phoneNumber =
        ...
        response.instance?.ownerJid ||
        response.instance?.phoneNumber ||
        response.instance?.wuid ||
        response.owner;
      ...
      if (
        !phoneNumber &&
        this.mapStatus(state) === EnumInstanceStatus.CONNECTED
      ) {
        try {
          const allInstances = await this.request(
            'GET',
            '/instance/fetchInstances',
          );
          ...
          if (matched) {
            ...
            this.logger.log(
              `Fallback phoneNumber recovery success for ${instanceName}: ${phoneNumber}`,
            );
          }
        } catch (fallbackErr) {
      ...
      this.logger.warn(
        `Failed to get status for ${instanceName}: ${error.message}`,
      );
      const isNotFound =
        error.message?.includes('404') || error.message?.includes('not exist');
      return {
  ...
  async sendMedia(
    ...
    if (media.type === 'audio') {
      ...
      this.logger.log(
        `Sent WhatsApp Voice Note (PTT) to ${to} via Evolution API`,
      );
    ...
    const extension =
      ...
      (media.type === 'video' ? 'mp4' : media.type === 'image' ? 'jpg' : 'pdf');
  ...
  async sendFlow(
    ...
    const flowToken = flowConfig.flowToken || `flow-token-${Date.now()}`;
    const flowScreen = flowConfig.flowScreen || 'UNION';

    const response = await this.request(
      'POST',
      `/message/sendInteractive/${instanceName}`,
      {
        number: resolvedNumber,
        type: 'flow',
        flow: {
          header: '',
          body: flowConfig.messageText,
          footer: '',
          flowId: flowConfig.flowId,
          flowToken: flowToken,
          flowButtonText: flowConfig.flowButtonText,
          flowAction: 'navigate',
    ...
    this.logger.log(
      `Sent WhatsApp Flow (${flowConfig.flowId}) to ${to} via Evolution API`,
    );
  ...
  private async resolveAndValidateNumber(
    instanceName: string,
    phone: string,
  ): Promise<string | null> {
    ...
    try {
      ...
      if (Array.isArray(response) && response.length > 0) {
        ...
        if (formatted.startsWith('55')) {
          ...
          // Se tem 12 dígitos (sem o 9), tenta com o 9
            ...
            fallbackNumber =
              formatted.substring(0, 4) + '9' + formatted.substring(4);
          }

          if (fallbackNumber) {
            this.logger.log(
              `Number ${formatted} not found on WhatsApp. Trying fallback format ${fallbackNumber}...`,
            );
            try {
              ...
              if (
                ...
                fallbackResponse.length > 0
              ) {
                const fallbackInfo = fallbackResponse[0];
                if (fallbackInfo.exists === true) {
                  this.logger.log(
                    `Fallback successful! Using ${fallbackInfo.number || fallbackNumber}`,
                  );
                  if (fallbackInfo.jid) {
      ...
      this.logger.error(
        `Error resolving number ${phone} on ${instanceName}: ${error.message}`,
      );
  ...
  async getMaturityMetrics(
    instanceName: string,
  ): Promise<{ chatCount: number; groupCount: number }> {
    try {
      ...
      this.logger.log(
        `Maturity Metrics for ${instanceName}: ${chatCount} private chats, ${groupCount} groups`,
      );

      return { chatCount, groupCount };
    } catch (error: any) {
      this.logger.error(
        `Failed to scan maturity for ${instanceName}: ${error.message}`,
      );
      return { chatCount: 0, groupCount: 0 };
    }
  }
  ...
  async joinGroup(instanceName: string, inviteUrl: string): Promise<any> {
    try {
      this.logger.log(
        `[Evolution] Joining group via invite: ${inviteUrl} for instance ${instanceName}`,
      );
      ...
      this.logger.error(
        `Failed to join group for ${instanceName}: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async getGroupParticipants(
    ...
    { id: string; name?: string; groupName: string; username?: string }[]
  > {
    try {
      ...
      this.logger.log(
        `Found ${participants.length} group participants in ${instanceName}`,
      );
      return participants;
    } catch (error: any) {
      this.logger.error(
        `Failed to get Evolution group participants: ${error.message}`,
      );
      return [];
    }
  }
