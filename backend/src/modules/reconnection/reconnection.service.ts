...
export class ReconnectionService {
  ...
  async findEligibleInstances(limit: number): Promise<Instance[]> {
    ...
    return this.instanceRepo
      ...
      .andWhere('instance.status IN (:...statuses)', {
        statuses: [InstanceStatus.DISCONNECTED, InstanceStatus.ERROR],
      })
      .andWhere(
  ...
  async processInstance(instance: Instance): Promise<void> {
    this.logger.debug(
      `Processing reconnection for ${instance.instanceName} (${instance.id})`,
    );
    ...
    try {
      ...
      if (status.status === InstanceStatus.CONNECTED) {
        this.logger.log(
          `Instance ${instance.instanceName} recovered! Provider says CONNECTED.`,
        );
      ...
      this.logger.log(
        `Attempting reconnection #${instance.reconnectAttempts} for ${instance.instanceName} (Status: ${status.status})`,
      );
  ...
  private async handleReconnectError(instance: Instance, error: any) {
    this.logger.warn(
      `Reconnection failed for ${instance.instanceName}: ${error.message}`,
    );
    ...
    updateData.reconnectLockedUntil = new Date(
