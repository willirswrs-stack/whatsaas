...
export class EvolutionApiService {
    ...
    try {
      ...
      try {
        ...
        if (createError.message?.includes('already exists')) {
          this.logger.log(
            `Session '${sessionName}' already exists, using existing`,
          );
        } else {
          throw createError;
        }
    ...
    try {
      ...
      for (let i = 0; i < maxRetries; i++) {
        try {
          ...
          this.logger.warn(
            `No QR code in response: ${JSON.stringify(response)}`,
          );
          return '';
        } catch (qrError: any) {
          // Se a sessão ainda está iniciando, aguardar e tentar novamente
          if (
            qrError.message?.includes('STARTING') ||
            qrError.message?.includes('Try again')
          ) {
            this.logger.log(
              `Session still starting, retry ${i + 1}/${maxRetries}...`,
            );
      ...
      throw new Error(
        'Timeout waiting for QR Code - session did not enter SCAN_QR_CODE state',
      );
    } catch (error: any) {
  ...
  async sendText(
    ...
    const response = await this.request('POST', `/api/sendText`, {
      session: sessionName,
      chatId: this.formatJid(to),
      text,
    });
  ...
  async getContacts(instanceName: string): Promise<any[]> {
    ...
    try {
      this.logger.log(
        `Fetching contacts from WAHA API session: ${sessionName}`,
      );
      ...
      this.logger.warn(
        `Unexpected format for contacts response: ${JSON.stringify(response).substring(0, 200)}`,
      );
      return [];
    } catch (error) {
