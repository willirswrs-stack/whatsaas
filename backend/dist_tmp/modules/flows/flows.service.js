...
const meta_graph_api_service_1 = require("../meta-templates/meta-graph-api.service");
const active_prevention_service_1 = require("../anti-ban/active-prevention.service");
const human_behavior_service_1 = require("../anti-ban/human-behavior.service");
...
let FlowsService = class FlowsService {
    ...
    inboxService;
    eventsGateway;
    constructor(flowRepository, executionRepository, triggerRepository, campaignContactRepository, flowQueue, instancesService, contactsService, whatsappFactory, aiService, configService, metaGraphApiService, activePrevention, humanBehavior, inboxService, eventsGateway) {
        ...
        this.activePrevention = activePrevention;
    ...
    async findById(tenantId, id) {
        ...
        try {
            ...
            const sentStats = await this.executionRepository.query(`SELECT log->>'nodeId' as node_id, count(*) as count 
                 FROM flow_executions, jsonb_array_elements(logs) as log 
                 WHERE flow_id = $1 
                 GROUP BY log->>'nodeId'`, [id]);
            // Create stats map
            const statsMap = {};
            ...
            executingStats.forEach(stat => {
                ...
                if (!statsMap[nodeId])
                    statsMap[nodeId] = { executing: 0, sent: 0 };
    ...
    async activate(tenantId, id) {
        ...
        if (!flow.nodes || flow.nodes.length === 0) {
            throw new common_1.BadRequestException('O fluxo precisa ter pelo menos um nó');
        }
    ...
    // ============ TRIGGERS ============
    async createTrigger(tenantId, dto) {
    ...
    async getTriggersByFlow(flowId) {
        return this.triggerRepository.find({ where: { flowId } });
    }
    // ============ EXECUTIONS ============
    async startExecution(tenantId, dto) {
        const flow = await this.findById(tenantId, dto.flowId);
        if (flow.status !== 'active') {
            throw new common_1.BadRequestException('O fluxo precisa estar ativo para ser executado');
        }
        // Find start node
        const startNode = flow.nodes.find(n => n.data.type === 'start');
        if (!startNode) {
            throw new common_1.BadRequestException('O fluxo não tem um nó de início');
        }
     ...
     */
    findNextEdge(edges, currentNodeId, preferredHandle) {
    ...
    async updateCampaignContactMessageId(execution, messageId) {
    ...
    async markCampaignContactAsSent(execution) {
        const campaignContactId = execution.variables?.campaignContactId;
    ...
    async checkCampaignCompletion(campaignId) {
        try {
            const campaignRepo = this.campaignContactRepository.manager.getRepository(campaign_entity_1.Campaign);
     ...
     */
    async getExecutionContext(execution) {
        ...
        try {
            ...
            if (isLocalhost || isPrivateIp) {
                ...
                if (parsed.port === String(port) || (!parsed.port && port === 80)) {
                    ...
                    if (isProd) {
                        const uploadsBase = this.configService.get('UPLOADS_BASE_URL');
                        if (uploadsBase) {
                            try {
    ...
    async processExecution(executionId, processCurrentNode = false) {
        // 1. Fetch current state
        const execution = await this.executionRepository.findOne({ where: { id: executionId } });
        if (!execution || execution.status !== 'running') {
            if (execution)
                console.log(`[Flow] Execution ${executionId.slice(0, 8)} skipped (status: ${execution.status})`);
            return;
        }
        ...
        if (!nextNode) {
            console.error(`[Flow] Next node not found for execution ${executionId} (currentNodeId: ${currentNodeId})`);
            execution.status = 'completed';
            execution.completedAt = new Date();
            await this.executionRepository.save(execution);
            await this.markCampaignContactAsSent(execution);
        ...
        console.log(`[Flow] [${executionId.slice(0, 8)}] ▶ Processing node: ${nextNode.id} | type: ${nodeType} | label: ${nextNode.data?.label || '-'} | from edge: ${edgeId} (handle: ${sourceHandle})`);
        ...
        try {
            ...
            if (aiNodeTypes.includes(nodeType)) {
                ...
                if (instance?.status === 'connected' && contact) {
                    const responseContent = await this.aiService.generateResponseWithKey(enhancedSystemPrompt, userMessage, apiKey, 'openai');
            ...
            // ==================== BUTTONS NODES (buttonsDefault, buttonsCopy, buttonsActions) ====================
                ...
                if (instance?.status === 'connected' && contact) {
                    ...
                    try {
                        ...
                        if (instance.provider === 'evolution_cloud' && provider.sendButtons) {
                            const res = await provider.sendButtons(instance.instanceName, contact.phone, {
                                text: message,
                            ...
                            if (buttons.length > 0) {
                                const buttonList = buttons.map((b, i) => `${i + 1}. ${b.label || b.text || b}`).join('\n');
            ...
            // ==================== DELAY NODE ====================
                ...
                await this.executionRepository.save(execution);
                await this.flowQueue.add('resume', { executionId }, {
                    delay: delayMs,
                ...
                if (instance?.status === 'connected' && contact && templateName) {
                    try {
                        ...
                        if (instance.channelType === 'official' && instance.metaConfig?.phoneNumberId && instance.metaConfig?.accessToken) {
                            ...
                            const res = await this.metaGraphApiService.sendMessage(instance.metaConfig.phoneNumberId, instance.metaConfig.accessToken, contact.phone, 'template', {
                                name: templateName,
            ...
            // ==================== AI NODES (ChatGPT, Gemini, etc) ====================
                ...
                if (token && contact) {
                    try {
                        ...
                        const response = await this.aiService.generateResponseWithKey(prompt, userMessage, token, aiProvider);
            ...
            // ==================== RANDOMIZER NODE ====================
                ...
                if (outEdges.length > 0) {
                    const nodeData = nextNode.data;
                    const configPaths = nodeData?.config?.paths || [];
                    let selectedEdge;
                    let selectedIndex;
                    let selectionMethod;
                    let weightsUsed;
                    ...
                    if (hasWeights) {
                        ...
                        if (totalWeight > 0) {
                            ...
                            let pathIndex = -1;
                            if (selectedEdge.sourceHandle && selectedEdge.sourceHandle.startsWith('path-')) {
                                pathIndex = parseInt(selectedEdge.sourceHandle.replace('path-', ''), 10);
                            }
            ...
            if (execution.variables?.campaignContactId) {
                try {
                    await this.campaignContactRepository.update(execution.variables.campaignContactId, { status: 'failed', errorMessage: error.message });
    ...
    async getExecutions(tenantId, flowId) {
    ...
    // ============ TESTING ============
    async testFlow(tenantId, flowId, phone, instanceId) {
        ...
        if (!instance) {
            throw new common_1.BadRequestException('Nenhuma instância conectada para envio.');
        }
        ...
        if (!contact) {
            throw new common_1.BadRequestException('Erro ao criar ou encontrar contato de teste.');
        }
        // Executar
        ...
        if (!execution) {
            throw new common_1.BadRequestException('Erro ao iniciar execução de teste.');
        }
        return {
    ...
    isWithinBusinessHours(config) {
        if (!config || !config.enabled || !config.days)
            return true;
        const now = new Date();
        const daysMap = {
        ...
        const dayConfig = config.days.find((d) => d.id === currentDayId);
        if (!dayConfig)
    ...
    async checkFlowTriggers(tenantId, instanceId, contactId, message) {
        ...
        for (const flow of activeFlows) {
            ...
            const hasMatch = triggers.some((t) => {
                if (t.type === 'keyword') {
                    ...
                    if (t.match === 'exact')
                        return normalizedMsg === val;
                    if (t.match === 'contains')
                        return normalizedMsg.includes(val);
                    if (t.match === 'starts_with')
                        return normalizedMsg.startsWith(val);
                }
                return false;
            });
            if (hasMatch) {
...
exports.FlowsService = FlowsService = __decorate([
    ...
    __param(0, (0, typeorm_1.InjectRepository)(entities_1.Flow)),
    __param(1, (0, typeorm_1.InjectRepository)(entities_1.FlowExecution)),
    __param(2, (0, typeorm_1.InjectRepository)(entities_1.FlowTrigger)),
    __param(3, (0, typeorm_1.InjectRepository)(campaign_entity_1.CampaignContact)),
    __param(4, (0, bullmq_1.InjectQueue)(bull_config_1.FLOW_QUEUE)),
    __param(8, (0, common_1.Inject)(ai_service_1.AiService)),
    __metadata("design:paramtypes", [typeorm_2.Repository,
        ...
        meta_graph_api_service_1.MetaGraphApiService,
        active_prevention_service_1.ActivePreventionService,
        human_behavior_service_1.HumanBehaviorService,
